...
cdef string tostring(data) except *:
  ...
  if isinstance(data, str):
    return data
  # TODO(wolfsonkin): Remove this temporary cludge (branch) allowing Python 2
  # string compatibility for pywrapfst users when all Python 2 users are gone.
  elif sys.version_info < (3, 0) and isinstance(data, bytes):
    return data
  raise TypeError(f"Expected {str.__name__} but received {type(data).__name__}: {data!r}")
...
cpdef SymbolTable compact_symbol_table(SymbolTableView syms):
  """
...
cdef class _SymbolTableIterator:
  ...
  def __init__(self, SymbolTableView syms):
    self._table = syms
...
cdef class EncodeMapper:
  ...
  def set_input_symbols(self, SymbolTableView syms):
    """
  ...
  def set_output_symbols(self, SymbolTableView syms):
    """
