...
std::optional<int64_t> StringCompiler::NumericalSymbolToLabel(
  ...
  const bool negate = absl::ConsumePrefix(&token, "-");
  const int base = [&token]() {
    if (absl::ConsumePrefix(&token, "0x") ||
        absl::ConsumePrefix(&token, "0X")) {
      return 16;  // Hex string
    } else if (token == "0" || absl::ConsumePrefix(&token, "0")) {
...
int64_t StringCompiler::StringSymbolToLabel(absl::string_view token) {
  // Is a single byte.
