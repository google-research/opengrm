...
#ifndef OPENGRM_STRING_STRINGCOMPILE_H_
#define OPENGRM_STRING_STRINGCOMPILE_H_
...
constexpr absl::string_view kGeneratedSymbolsName = "**Generated symbols";
constexpr absl::string_view kEpsilonString = "<epsilon>";
...
constexpr absl::string_view kBosString = "BOS";
constexpr absl::string_view kEosString = "EOS";
...
class StringCompiler {
 public:
  ...
  bool StringToLabels(absl::string_view str, std::vector<Label>* labels,
    ...
    switch (token_type) {
      ...
      case TokenType::SYMBOL: {
        ...
        for (const auto token : absl::StrSplit(str, ' ')) {
  ...
  template <class Arc>
  bool Compile(absl::string_view str, MutableFst<Arc>* fst,
               TokenType token_type = TokenType::BYTE,
               const SymbolTable* symbols = nullptr,
 ...
 private:
  ...
  std::optional<int64_t> NumericalSymbolToLabel(absl::string_view token) const;
  int64_t StringSymbolToLabel(absl::string_view token);
  int64_t NumericalOrStringSymbolToLabel(absl::string_view token);

  // Processes a BYTE or a UTF8 span inside brackets.
  template <class Label>
  bool ProcessBracketedSpan(absl::string_view span,
                            std::vector<Label>* labels) {
    const std::vector<absl::string_view> tokens = absl::StrSplit(span, ' ');
    if (tokens.empty()) {
  ...
  template <class Label>
  bool ProcessUnbracketedSpan(absl::string_view span,
  ...
  template <class Arc>
  void LabelsToFst(absl::Span<const typename Arc::Label> labels,
                   MutableFst<Arc>* fst,
                   typename Arc::Weight weight = Arc::Weight::One()) {
...
template <class Label>
ABSL_MUST_USE_RESULT bool StringToLabels(absl::string_view str,
                                         std::vector<Label>* labels,
                                         TokenType token_type = TokenType::BYTE,
                                         const SymbolTable* symbols = nullptr) {
...
template <class Arc>
ABSL_MUST_USE_RESULT bool StringCompile(
    absl::string_view str, MutableFst<Arc>* fst,
    TokenType token_type = TokenType::BYTE,
    const SymbolTable* symbols = nullptr,
    typename Arc::Weight weight = Arc::Weight::One()) {
  static auto* compiler = internal::StringCompiler::Get();
...
#endif  // OPENGRM_STRING_STRINGCOMPILE_H_
