...
#ifndef OPENGRM_STRING_STRING_VIEW_FST_H_
#define OPENGRM_STRING_STRING_VIEW_FST_H_
...
class ArcIterator<StringViewFst<Arc, Viewer>> : public ArcIteratorBase<Arc> {
 public:
  ...
  void Seek(size_t s) final { done_ = (s != 0) || !has_arcs_; }
...
class StringViewFst
 ...
 public:
  ...
  StringViewFst(const StringViewFst<Arc, Viewer>& fst, bool safe = false)
      : ImplToExpandedFst<Impl>(std::make_shared<Impl>(fst.GetImpl()->view())) {
  }
...
#endif  // OPENGRM_STRING_STRING_VIEW_FST_H_
