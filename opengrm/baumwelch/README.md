...
Training is performed using the `Train` function. It takes as arguments a FAR or
WFST representing the plaintext, and channel model and a FAR representing the
ciphertext.
...
[*path property*](http://www.openfst.org/twiki/bin/view/FST/FstWeightRequirements),
...
`Train` takes an struct representing options for training. These define the size
of the batch, the maximum number of iterations for training, the step size
reduction power $\alpha$, and the comparison/quantization $\delta$ used to
detect convergence.

Since expectation maximization training is only locally optimal, *random
(re)starts* can be used to avoid local minima. For particularly difficult
...
the desired semiring. `Randomize` can be used to generate random restarts.
...
expectation at time $k$. Interpolation is disabled when $\alpha = 0$.

## Decoding

`Decode` is the primary driver for decoding. Decoding with the trained model is
essentially a process of constructing a weighted lattice followed by a shortest
path computation.
...
normally want to preserve the input-output correspondences. In the decipherment
...
Therefore we use the following algorithm due to Gorman & Allauzen (2024):
...
5.  Compute the shortest path of the on-the-fly tropical-semiring DFA using A*
    search with $\beta_d$ as a heuristic, halting immediately after the
    shortest path is found. This gives us the shortest string.
...
1.  Compute the best $i, o$ alignments by decoding with $\hat{\Gamma}$, then
    reinterpret the FSTs in the decoded alignments FAR as acceptors over a pair
    symbol vocabulary ${I} \times {O}$.
2.  Compute a higher-order language model (e.g., using the NGram tools), with
    standard smoothing and shrinking options, producing a weighted finite-state
    acceptor.
3.  Convert the weighted finite-state acceptor to a finite-state transducer by
    reinterpreting the "acceptor" ${I} \times {O}$ arcs with the corresponding
    "transducer" ${I} : {O}$ arcs.
...
restarts. In *Proceedings of the 2013 Conference on Empirical Methods in Natural
Language Processing*, pages 874-878.
...
Gorman, K. and Allauzen, C. 2024. A* shortest string decoding for non-idempotent
semirings. In *Proceedings of the 18th Conference of the European Chapter of the
Association for Computational Linguistics*, pages 732-739.

Gorman, K., Kirov, C., Roark, B., and Sproat, R. 2021. Structured abbreviation
expansion in context. In *Findings of the Association for Computational
...
decipherment problems. In *Proceedings of the COLING/ACL 2006 Main Conference
Poster Sessions*, pages 499-506.
...
Natural Language Processing*, pages 45-49.
