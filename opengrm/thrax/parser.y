...
grammar:
  import_list func_or_stmt_list
    { GrammarNode* node = new GrammarNode(absl::WrapUnique($1), absl::WrapUnique($2->funcs_), absl::WrapUnique($2->stmts_));
      delete $2;
      $$ = node;
      parm->SetAst(absl::WrapUnique(absl::implicit_cast<Node*>($$))); }
| error
...
| stmt func_or_stmt_list
    { $2->stmts_->AddFront(absl::WrapUnique($1));
      $$ = $2; }
| func_decl func_or_stmt_list
    { $2->funcs_->AddFront(absl::WrapUnique($1));
      $$ = $2; }
;
...
return_stmt:
  tKEYWORD_RETURN obj tSEMICOLON
    { ReturnNode* node = new ReturnNode(absl::WrapUnique($2));
      $$ = node; }
;

rule_stmt:
  descriptor tEQUALS rule_body tSEMICOLON
    { RuleNode* node = new RuleNode(absl::WrapUnique($1), absl::WrapUnique($3),
                                    absl::GetFlag(FLAGS_always_export) ?
                                    RuleNode::EXPORT :
                                    RuleNode::DO_NOT_EXPORT);
      $$ = node; }
| tKEYWORD_EXPORT descriptor tEQUALS rule_body tSEMICOLON
    { RuleNode* node = new RuleNode(absl::WrapUnique($2), absl::WrapUnique($4), RuleNode::EXPORT);
      $$ = node; }
;
...
descriptor:
  tDESCR
    { const std::string& name = parm->GetLexer()->YYString();
      ...
      node->SetLine(parm->GetLexer()->line_number());
      if (!node->IsValid())
        parm->Error(absl::StrCat("Illegal identifier: ", name));
      $$ = node; }
;
...
concat_fst:
  repetition_fst concat_fst %prec tCONCAT
    { FstNode* node = new FstNode(FstNode::CONCAT_FSTNODE);
      node->SetLine(parm->GetLexer()->line_number());
      node->AddArgument(absl::WrapUnique($1));
      node->AddArgument(absl::WrapUnique($2));
      $$ = node; }
| repetition_fst
    { $$ = $1; }
;
...
union_fst:
  composition_fst tPIPE union_fst
    { FstNode* node = new FstNode(FstNode::UNION_FSTNODE);
      node->SetLine(parm->GetLexer()->line_number());
      node->AddArgument(absl::WrapUnique($1));
      node->AddArgument(absl::WrapUnique($3));
      $$ = node; }
| composition_fst
    { $$ = $1; }
;
...
funccall_arguments:
  obj
    { CollectionNode* node = new CollectionNode();
      node->AddFront(absl::WrapUnique($1));
      $$ = node; }
| obj tCOMMA funccall_arguments
    { $3->AddFront(absl::WrapUnique($1));
      $$ = $3; }
// TODO: These are only needed by StringFile. There should be a better way
// to do this.
| tKEYWORD_BYTE
    { CollectionNode* node = new CollectionNode();
      StringNode* nnode = new StringNode("byte");
      node->AddFront(absl::WrapUnique(nnode));
      $$ = node; }
| tKEYWORD_BYTE tCOMMA funccall_arguments
    { StringNode* nnode = new StringNode("byte");
      $3->AddFront(absl::WrapUnique(nnode));
      $$ = $3; }
| tKEYWORD_UTF8
    { CollectionNode* node = new CollectionNode();
...
| atomic_obj tLBRACE number tCOMMA number tRBRACE
    { if ($3 > $5)
        parm->Error(absl::StrCat("Reversed repetition bounds: ", $3, " > ", $5));
      if ($3 < 0)
        parm->Error(absl::StrCat("Start bound must be non-negative: ", $3));
      if ($5 < 0)
        parm->Error(absl::StrCat("End bound must be non-negative: ", $5));
...
| atomic_obj tLBRACE number tRBRACE
    { if ($3 < 0)
        parm->Error(absl::StrCat("Repetition count should be non-negative: ", $3));
      RepetitionFstNode* node = new RepetitionFstNode(RepetitionFstNode::RANGE);
      node->AddArgument(absl::WrapUnique($1));
      node->SetRange($3, $3);
      $$ = node; }
...
fst_with_weight:
  ...
  { if (!absl::down_cast<FstNode*>($1)->SetWeight(absl::WrapUnique($2)))
...
import_request:
  tKEYWORD_IMPORT quoted_string tKEYWORD_AS descriptor tSEMICOLON
    { ImportNode* node = new ImportNode(absl::WrapUnique($2), absl::WrapUnique($4));
      node->SetLine(parm->GetLexer()->line_number());
      $$ = node; }
| error tSEMICOLON
...
func_decl:
  tKEYWORD_FUNC descriptor func_arglist func_body
    { FunctionNode* node = new FunctionNode(absl::WrapUnique($2), absl::WrapUnique($3), absl::WrapUnique($4));
  ...
  switch(parm->GetLexer()->YYLex()) {
    ...
    case Lexer::CONNECTOR: {
      ...
      if (connector.length() != 1) {
        parm->Error(absl::StrCat("Parse error - unknown connector: ", connector));
        return 0;
      }
      switch (parm->GetLexer()->YYString()[0]) {
        ...
        case '}': return tRBRACE;
        case '|': return tPIPE;
        default:  parm->Error(absl::StrCat("Parse error - unknown connector: ", connector));
                  return 0;
      }
    }
    case Lexer::KEYWORD: {
      ...
      if (keyword == "export") {
        ...
        parm->Error(absl::StrCat("Parse error - unknown keyword: ", keyword));
        return 0;
      }
    }
  }
