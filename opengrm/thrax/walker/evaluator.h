...
#ifndef OPENGRM_THRAX_WALKER_EVALUATOR_H_
#define OPENGRM_THRAX_WALKER_EVALUATOR_H_
...
#include "opengrm/thrax/walker/util/register.h"
#include "opengrm/thrax/walker/walker.h"


ABSL_DECLARE_FLAG(bool, always_export);
ABSL_DECLARE_FLAG(bool, optimize_all_fsts);
ABSL_DECLARE_FLAG(bool, print_rules);
ABSL_DECLARE_FLAG(bool, save_symbols);
...
const function::Function<Arc>* GetFunction(absl::string_view func_name) {
...
class AstEvaluator : public AstWalker {
 public:
  ...
  using LabelMapper = absl::flat_hash_map<int64_t, int64_t>;
  ...
  // Two functions to control error handling in the case of bad input grammars.

  bool Success() const { return success_; }

  void Error(const Node& node, absl::string_view message) {
  ...
  void Visit(GrammarNode* node) override {
    ...
    if (run_all_) {
      ...
      for (int i = 0; i < statements->Size(); ++i) {
        StatementNode* stmt = absl::down_cast<StatementNode*>((*statements)[i]);
  ...
  void Visit(ImportNode* node) override {
    ...
    const auto& path =
        fst::JoinPath(absl::GetFlag(FLAGS_indir), node->GetPath()->Get());
    if (fst::Extension(path) != "grm") {
      Error(*node, absl::StrCat("Extension for included files should be .grm: ",
                                path));
      return;
    }
    // Gets (and check) the alias name.
    const std::string& alias = node->GetAlias()->Get();
    if (absl::StrContains(alias, '.')) {
      Error(*node, absl::StrCat("Invalid import alias identifier: ", alias));
      return;
    }
    ...
    if (!grammar->ParseFile(path) ||
      ...
      Error(*node,
            absl::StrCat("Errors while importing grm source file: ", path));
      env_ = prev_env;
      return;
    }
    ...
    const std::string far_path =
        absl::StrCat(path.substr(0, path.length() - 3), "far");
    VLOG(2) << "Opening (and loading FSTs from) companion far: " << far_path;

    std::unique_ptr<::fst::STTableFarReader<Arc>> far_reader;
    if (absl::StatusOr<std::unique_ptr<::fst::STTableFarReader<Arc>>>
            far_reader_statusor =
                ::fst::STTableFarReader<Arc>::OpenWithStatus(far_path);
        far_reader_statusor.ok()) {
      far_reader = *std::move(far_reader_statusor);
    } else {
      Error(*node, absl::StrCat("Unable to open far archive at ", far_path,
                                ": ", far_reader_statusor.status().message()));
    ...
    if (Success() && !far_reader->Done() &&
        far_reader->Find(kStringFstSymtabFst)) {
    ...
    for (/* far_reader starts at the beginning */;
      ...
      if (key == kStringFstSymtabFst) {
        ...
        if (!env_->Get<DataType>(key_inode)) {  // Add only if new.
          ...
          auto fst = absl::WrapUnique(tmpfst.Copy());
          bool new_add =
  ...
  void Visit(RuleNode* node) override {
    ...
    if (absl::GetFlag(FLAGS_print_rules)) {
      std::cout << "Evaluating rule: " << identifier->Get() << std::endl;
    }
    if (identifier->HasNamespaces()) {
      Error(*identifier,
            absl::StrCat("Cannot assign to an identifier within a namespace: ",
                         identifier->Get()));
      return;
    }
    ...
    if (!env_->InsertLocal(name, std::move(thing))) {
      Error(*identifier,
            absl::StrCat("Cannot clobber existing variable: ", name));
      return;
    }
    if (node->ShouldExport()) {
      if (env_->LocalEnvironmentDepth() == 1) {
        exported_fsts_.insert(identifier);
      } else if (!absl::GetFlag(FLAGS_always_export)) {
        Error(*identifier,
              absl::StrCat("Variables may only be exported from the top-level "
                           "grammar: ",
                           name));
        return;
      }
  ...
  void GetFsts(absl::btree_map<std::string, std::unique_ptr<const Transducer>,
    ...
    for (const auto* fst_i : exported_fsts_) {
      ...
      if (env_->Get<DataType>(*fst_i) == nullptr) {
        LOG(WARNING) << "Cannot find exportable fst with name " << name
                     << ": ignoring.";
        continue;
      }
      ...
      if (!env_->Get<DataType>(*fst_i)->template is<Transducer*>()) {
        Error(*fst_i,
              absl::StrCat("Cannot export non-FST variable: ", fst_i->Get()));
        return;
      }
      Transducer* fst =
      ...
      //
      // TODO: The particular example in b/119868645 is evidently fixed by
  ...
  void set_file(absl::string_view file) { file_ = file; }
 ...
 private:
  ...
  std::unique_ptr<DataType> MakeFstFromLocalFunction(
    ...
    if (fa_node->Size() != arguments->size()) {
      Error(debug_location_node,
            absl::StrCat("Expected ", fa_node->Size(), " arguments but got ",
                         arguments->size()));
    }
    for (int i = 0; Success() && i < fa_node->Size(); ++i) {
      IdentifierNode* fa_identifier =
          absl::down_cast<IdentifierNode*>((*fa_node)[i]);
      if (fa_identifier->HasNamespaces()) {
        Error(*fa_identifier, absl::StrCat("Invalid function argument: ",
                                           fa_identifier->Get()));
        break;
      }
    ...
    for (int i = 0; Success() && i < fb_node->Size(); ++i) {
      StatementNode* stmt = absl::down_cast<StatementNode*>((*fb_node)[i]);
      stmt->Accept(this);
      if (stmt->GetType() == StatementNode::RETURN_STATEMENTNODE) {
  ...
  std::unique_ptr<DataType> MakeFstFromCFunction(
      absl::string_view function_name, const Node& debug_location_node,
      std::unique_ptr<std::vector<std::unique_ptr<DataType>>> arguments) {
  ...
  std::unique_ptr<DataType> MakeFst(FstNode* node) {
    ...
    switch (node->GetType()) {
      ...
      case FstNode::REPETITION_FSTNODE: {
        ...
        RepetitionFstNode* rnode = absl::down_cast<RepetitionFstNode*>(node);
      ...
      case FstNode::IDENTIFIER_FSTNODE: {
        IdentifierNode* identifier =
            absl::down_cast<IdentifierNode*>(node->GetArgument(0));
        VLOG(2) << "Identifier Fst: " << identifier->Get();
        DataType* original = env_->Get<DataType>(*identifier);
        if (!original) {
          Error(*identifier,
                absl::StrCat("Undefined symbol: ", identifier->Get()));
          return nullptr;
        }
        ...
        // is the final access.
        if (env_->IsTopLevel() && env_->LocalEnvironmentDepth() == 1 &&
      ...
      case FstNode::STRING_FSTNODE: {
        StringFstNode* snode = absl::down_cast<StringFstNode*>(node);
        const std::string& text =
            absl::down_cast<StringNode*>(node->GetArgument(0))->Get();
      ...
      case FstNode::FUNCTION_FSTNODE: {
        ...
        auto args = GetArgumentsFromCollectionNode(
            absl::down_cast<CollectionNode*>(node->GetArgument(1)));
        if (!Success() || !args) {
          Error(*func_identifier_node,
                absl::StrCat("Unable to bind all arguments for function call: ",
        ...
        if (func_node) {
          ...
          // function.
          DCHECK_EQ(func_node->GetName()->Get(),
        ...
        if (!function_found) {
          Error(*func_identifier_node,
                absl::StrCat("Undefined function identifier: ",
                             func_identifier_node->Get()));
          return nullptr;
        }
    ...
    if (output && output->is<Transducer*>()) {
      ...
      // FUNCTION_FSTNODE).
      if (absl::GetFlag(FLAGS_optimize_all_fsts) || node->ShouldOptimize()) {
  ...
  static std::unique_ptr<Transducer> AttachWeight(
      const Transducer& input_fst, absl::string_view weight_str) {
    // Generates the appropriate weight.
    auto weight = ::fst::StrToWeight<typename Arc::Weight>(weight_str);
    // TODO: Investigate removing this branch.
  ...
  void ReassignSymbols(MutableTransducer* fst) {
    if (fst->InputSymbols()) {
      if (fst->InputSymbols()->Name() == function::kByteSymbolTableName) {
    ...
    if (fst->OutputSymbols()) {
      if (fst->OutputSymbols()->Name() == function::kByteSymbolTableName) {
  ...
  void RemapGeneratedLabels(MutableTransducer* fst) {
    for (::fst::StateIterator<MutableTransducer> siter(*fst); !siter.Done();
         siter.Next()) {
      for (::fst::MutableArcIterator<MutableTransducer> aiter(fst,
                                                              siter.Value());
  ...
  absl::flat_hash_set<IdentifierNode*> exported_fsts_;
  // A list of grammars that we've opened (and thus need to close at the end).
  // TODO: This is dangerous right now since if we have two simulatneous
  ...
  absl::flat_hash_set<std::string> observed_function_names_;

  AstEvaluator(const AstEvaluator<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_EVALUATOR_H_
