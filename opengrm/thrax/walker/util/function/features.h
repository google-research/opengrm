...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_FEATURES_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_FEATURES_H_
...
inline bool SplitFeatureValue(absl::string_view featval, std::string* feature) {
...
class Feature : public Function<Arc> {
  ...
  std::unique_ptr<DataType> Execute(
    ...
    if (args[i]->is<::fst::SymbolTable>()) {
      ...
      if (arg == "utf8") {
        ...
        if (absl::GetFlag(FLAGS_save_symbols)) symtab = GetUtf8SymbolTable();
      } else if (arg == "byte") {
        mode = ::fst::TokenType::BYTE;
        if (absl::GetFlag(FLAGS_save_symbols)) symtab = GetByteSymbolTable();
      } else {
    ...
    for (int i = 0; i < feature_values.size(); ++i) {
      ...
      int64_t label;
      if (!StringFst<Arc>::SymbolToGeneratedLabel(feature_value_pair, &label)) {
 ...
 private:
  ...
  Feature(const Feature<Arc>&) = delete;
...
class Category : public Function<Arc> {
 ...
 private:
  Category(const Category<Arc>&) = delete;
...
class FeatureVector : public Function<Arc> {
 public:
  ...
  ~FeatureVector() override = default;
 ...
 protected:
  std::unique_ptr<DataType> Execute(
    ...
    auto* category_fst =
        absl::down_cast<MutableTransducer*>(*args[0]->get<Transducer*>());
    std::vector<std::pair<typename Arc::StateId, std::string>> features;
    if (!Category<Arc>::ValidateFeatureSequenceFst(category_fst, &features)) {
      std::cout << "FeatureVector: First argument must be a Category fst"
                << std::endl;
      return nullptr;
    }
    ...
    std::unique_ptr<::fst::SymbolTable> generated_symbols(
        StringFst<Arc>::GetLabelSymbolTable(false));
    absl::btree_map<std::string, int64_t> feature_label_pairs;
 ...
 private:
  FeatureVector(const FeatureVector<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_FEATURES_H_
