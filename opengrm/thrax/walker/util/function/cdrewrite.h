...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_CDREWRITE_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_CDREWRITE_H_
...
class CDRewrite : public Function<Arc> {
 public:
  ...
  std::unique_ptr<DataType> Execute(
    ...
    if (absl::GetFlag(FLAGS_save_symbols)) {
      ...
      if (!::fst::CompatSymbols(sigma.InputSymbols(), sigma.OutputSymbols())) {
        std::cout << "CDRewrite: input symbols and output symbols must match "
                     "for sigma"
                  << std::endl;
        return nullptr;
      }
      if (!::fst::CompatSymbols(tau.InputSymbols(), lambda.InputSymbols())) {
    ...
    ::fst::CDRewriteDirection dir = ::fst::CDRewriteDirection::LEFT_TO_RIGHT;
    ::fst::CDRewriteMode mode = ::fst::CDRewriteMode::OBLIGATORY;
    if (args.size() > 4) {
      if (!args[4]->is<std::string>()) {
        std::cout << "CDRewrite: Expected string for argument 4" << std::endl;
        return nullptr;
      }
    ...
    ::fst::CDRewriteCompile(tau, lambda, rho, sigma, output.get(), dir, mode,
 ...
 private:
  CDRewrite(const CDRewrite<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_CDREWRITE_H_
