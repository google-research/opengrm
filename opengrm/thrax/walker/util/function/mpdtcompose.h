...
// (third_party/openfst/extensions/mpdt). The MPDT may be either be the first or
// second transducer, as controlled by the fourth (string) argument to the
...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_MPDTCOMPOSE_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_MPDTCOMPOSE_H_
...
class MPdtCompose : public Function<Arc> {
 ...
 protected:
  std::unique_ptr<DataType> Execute(
    ...
    if (!args[0]->is<Transducer*>() || !args[1]->is<Transducer*>() ||
        !args[2]->is<Transducer*>() || !args[3]->is<Transducer*>()) {
    ...
    if (absl::GetFlag(FLAGS_save_symbols)) {
      if (!::fst::CompatSymbols(left->OutputSymbols(), right->InputSymbols())) {
    ...
    ::rewrite::MakeParenthesesVector(parens_transducer, &parens);
    MutableTransducer assignments_transducer(**args[3]->get<Transducer*>());
    std::vector<Label> assignments;
    ::rewrite::MakeAssignmentsVector(assignments_transducer, parens,
                                     &assignments);
    // NB: In the underlying third_party/openfst/extensions/mpdt library we
    // actually just use left_pdt/right_pdt, but to keep things a little more
    // clear for the grammar writer, here we use left_mpdt/right_mpdt
    ...
    if (args.size() == 6) {
      ...
      if (sort_mode != "right") {
        ...
        left =
            new ::fst::ArcSortFst<Arc, ::fst::OLabelCompare<Arc>>(*left, ocomp);
        delete_left = true;
      }
      if (sort_mode != "left") {
        static const ::fst::ILabelCompare<Arc> icomp;
        right = new ::fst::ArcSortFst<Arc, ::fst::ILabelCompare<Arc>>(*right,
                                                                      icomp);
        delete_right = true;
      }
    }
    auto output = std::make_unique<MutableTransducer>();
    const ::fst::MPdtComposeOptions opts(/*connect=*/false);
    if (left_mpdt) {
      ::fst::Compose(*left, parens, assignments, *right, output.get(), opts);
    } else {
      ::fst::Compose(*left, *right, parens, assignments, output.get(), opts);
    }
 ...
 private:
  MPdtCompose(const MPdtCompose<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_MPDTCOMPOSE_H_
