...
#include "opengrm/thrax/walker/util/function/minimize.h"
...
TYPED_TEST_SUITE(MinimizeTest, ArcTypes, );

// Tests that the Minimize function correctly minimizes a deterministic FST
// by merging equivalent states.
TYPED_TEST(MinimizeTest, TestMinimize) {
  auto input = std::make_unique<typename TestFixture::MutableTransducer>();
  auto p = input->AddState();
  input->SetStart(p);
  auto q1 = input->AddState();
  auto q2 = input->AddState();
  input->EmplaceArc(p, 'a', 'a', q1);
  input->EmplaceArc(p, 'b', 'b', q2);
  input->SetFinal(q1);
  input->SetFinal(q2);

  auto expected = *input;
  ::fst::Minimize(&expected);
