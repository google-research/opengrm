...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_STRINGFST_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_STRINGFST_H_
...
class StringFst : public Function<Arc> {
 ...
 public:
  ...
  static std::unique_ptr<::fst::SymbolTable> GetLabelSymbolTable(
      bool top_level) {
    // TODO: Don't copy the generated symbols, though note that this
    ...
    auto symtab = absl::WrapUnique(::fst::GeneratedSymbols().Copy());
    ...
    if (absl::GetFlag(FLAGS_save_symbols) && top_level) {
  ...
  static bool MergeLabelSymbolTable(const ::fst::SymbolTable& symtab) {
    return ::fst::thrax_internal::MergeIntoGeneratedSymbols(symtab, &remap_);
  }
  ...
  // found.
  static bool SymbolToGeneratedLabel(absl::string_view symbol, int64_t* label) {
 ...
 private:
  ...
  StringFst(const StringFst<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_STRINGFST_H_
