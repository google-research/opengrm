...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_ASSERT_NULL_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_ASSERT_NULL_H_
...
// TODO: Some day we should make this so that it doesn't return a
...
class AssertNull : public UnaryFstFunction<Arc> {
 ...
 protected:
  std::unique_ptr<Transducer> UnaryFstExecute(
    ...
    if (args.size() != 1) {
      std::cout << "AssertNull: Expected 1 argument but got " << args.size()
                << std::endl;
      return nullptr;
    }
    ...
    if (mutable_left->NumStates() != 0) {
      std::cout << "Argument to AssertNull is not null:" << std::endl;
      return nullptr;
    }
    return mutable_left;
  }
 ...
 private:
  AssertNull(const AssertNull<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_ASSERT_NULL_H_
