...
// (third_party/openfst/extensions/pdt). The PDT may be either be the first or
...
#ifndef OPENGRM_THRAX_WALKER_UTIL_FUNCTION_PDTCOMPOSE_H_
#define OPENGRM_THRAX_WALKER_UTIL_FUNCTION_PDTCOMPOSE_H_
...
class PdtCompose : public Function<Arc> {
 ...
 protected:
  std::unique_ptr<DataType> Execute(
    ...
    const ::fst::PdtComposeOptions opts(/*connect=*/false);
    if (left_pdt) {
 ...
 private:
  PdtCompose(const PdtCompose<Arc>&) = delete;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_FUNCTION_PDTCOMPOSE_H_
