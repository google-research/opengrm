...
#ifndef OPENGRM_THRAX_WALKER_UTIL_RESOURCE_MAP_H_
#define OPENGRM_THRAX_WALKER_UTIL_RESOURCE_MAP_H_
...
class ResourceMap {
 private:
  struct Resource;
  using Map = absl::flat_hash_map<std::string, std::unique_ptr<Resource>>;
 ...
 public:
  ...
  template <typename T>
  bool Insert(absl::string_view name, std::unique_ptr<T> thing) {
  ...
  template <typename T>
  bool InsertWithDeleter(absl::string_view name, T* thing,
  ...
  // received.
  template <typename T>
  T* Get(absl::string_view name) const {
    absl::MutexLock lock(mutex_);
  ...
  // proper type.
  template <typename T>
  bool ContainsType(absl::string_view name) const {
    absl::MutexLock lock(mutex_);
    const auto it = map_.find(name);
  ...
  template <typename T>
  std::unique_ptr<T> Release(absl::string_view name) {
    absl::MutexLock lock(mutex_);
 ...
 private:
  ...
  template <typename T>
  void CheckType(Map::const_iterator it, absl::string_view name) const {
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_RESOURCE_MAP_H_
