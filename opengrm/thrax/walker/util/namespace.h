...
#ifndef OPENGRM_THRAX_WALKER_UTIL_NAMESPACE_H_
#define OPENGRM_THRAX_WALKER_UTIL_NAMESPACE_H_
...
class Namespace {
 public:
  ...
  Namespace* absl_nullable AddSubNamespace(absl::string_view filename,
  ...
  // pointer.
  template <typename T>
  bool Insert(absl::string_view identifier_name, std::unique_ptr<T> resource) {
    const std::string& name = ConstructMapName(identifier_name);
    return resources_->Insert(name, std::move(resource));
  }

  template <typename T>
  bool InsertWithoutDelete(absl::string_view identifier_name, T* resource) {
    const std::string& name = ConstructMapName(identifier_name);
  ...
  template <typename T>
  bool InsertLocal(absl::string_view identifier_name,
  ...
  bool EraseLocal(absl::string_view identifier);

  // Returns namespace in the identifier, according to the current iterator
  // position.
  const Namespace* absl_nullable ResolveNamespace(
      const IdentifierNode& identifier) const;
  Namespace* absl_nullable ResolveNamespace(const IdentifierNode& identifier);
 ...
 private:
  ...
  Namespace(absl::string_view filename, ResourceMap* resource_map);
  ...
  absl::flat_hash_map<std::string, std::unique_ptr<Namespace>>
      alias_namespace_map_;
...
#endif  // OPENGRM_THRAX_WALKER_UTIL_NAMESPACE_H_
