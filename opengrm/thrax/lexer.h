...
class Lexer {
 public:
  ...
  static const absl::btree_set<std::string> kKeywords;

  // Must be called before the grammar is processed via repeated calls to
  // YYLex().
  void ScanString(absl::string_view str) { grammar_.push(GrammarFile(str)); }
 ...
 private:
  ...
  struct GrammarFile {
    explicit GrammarFile(absl::string_view cont)
        : content(cont), pos(0), line_number(1) {}
    GrammarFile() : pos(0), line_number(1) {}
