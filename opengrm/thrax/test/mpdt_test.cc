...
class PdtTest : public ::testing::Test {
 protected:
  PdtTest() {
    ...
    testfile_path_ = fst::JoinPath(
        std::string("."),
        "opengrm/thrax/test/testdata/mpdt/mpdt.txt");
  }
...
TEST_F(PdtTest, FileTest) {
  ...
  while (testfile.ReadLine(&line)) {
    ...
    std::string rule_pair = pieces[0];
    std::vector<std::string> rule_pieces =
        absl::StrSplit(rule_pair, '$', absl::SkipEmpty());
    ASSERT_EQ(3, rule_pieces.size());
    const auto& rule = rule_pieces[0];
    const auto& parens = rule_pieces[1];
    const auto& assignments = rule_pieces[2];
    const auto& input = pieces[1];
    const auto& expected = pieces[2];

    std::string output;
    bool success =
        grm_->RewriteBytes(rule, input, &output, parens, assignments);
    bool equal = true;
    EXPECT_TRUE(success);
