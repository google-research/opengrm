 ...
 protected:
  PdtTest() {
    ...
    testfile_path_ = fst::JoinPath(
        std::string("."),
        "opengrm/thrax/test/testdata/pdt/pdt.txt");
  }
...
TEST_F(PdtTest, FileTest) {
  ...
  while (testfile.ReadLine(&line)) {
    ...
    std::string output;
    bool success = grm_->RewriteBytes(rule, input, &output, parens);
    bool equal = true;
    EXPECT_TRUE(success);
