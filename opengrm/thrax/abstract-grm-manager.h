...
#ifndef OPENGRM_THRAX_ABSTRACT_GRM_MANAGER_H_
#define OPENGRM_THRAX_ABSTRACT_GRM_MANAGER_H_
...
class AbstractGrmManager {
 public:
  ...
  static void StringifyFst(MutableTransducer* fst);
  ...
  // provided filename.
  virtual void ExportFar(absl::string_view filename) const = 0;
 ...
 protected:
  ...
  // Loads up the FSTs given the supplied reader.
  // TODO: Document requirements for `FarReader`.
  template <typename FarReader>
  absl::Status LoadArchiveWithStatus(FarReader& reader,
...
template <typename Arc>
AbstractGrmManager<Arc>::AbstractGrmManager() = default;
...
template <typename Arc>
template <typename FarReader>
absl::Status AbstractGrmManager<Arc>::LoadArchiveWithStatus(
    FarReader& reader, absl::string_view filename) {
  fsts_.clear();
  int num_read_fsts = 0;
  ...
  if (fsts_.size() != num_read_fsts) {
    return absl::InternalError(absl::StrCat(filename, ": ", num_read_fsts,
                                            " FSTs read but only ",
                                            fsts_.size(), " FSTs saved"));
  }
  if (fsts_.empty()) {
    return absl::InvalidArgumentError(absl::StrCat(
        filename, " is an empty FAR: Did you `export` any rules?"));
  }
  SortRuleInputLabels();
...
bool AbstractGrmManager<Arc>::LoadArchive(FarReader* reader,
                                          absl::string_view filename) {
  absl::Status status = LoadArchiveWithStatus(*reader, filename);
  if (!status.ok()) {
    LOG(ERROR) << status;
    return false;
  }
...
class RuleCascade {
 ...
 public:
  ...
  absl::Status InitWithStatus(const AbstractGrmManager<Arc>* grm,
                              std::vector<RuleTriple> rule_triples);

  [[deprecated("Use InitWithStatus instead.")]]
  bool Init(const AbstractGrmManager<Arc>* grm,
            std::vector<RuleTriple> rule_triples);

  // Parses the rule definitions and initializes the cascade.
  absl::Status InitFromDefsWithStatus(const AbstractGrmManager<Arc>& grm,
                                      absl::Span<const std::string> rules);

  [[deprecated("Use InitFromDefsWithStatus instead.")]]
...
template <typename Arc>
absl::Status RuleCascade<Arc>::InitWithStatus(
    const AbstractGrmManager<Arc>* grm, std::vector<RuleTriple> rule_triples) {
  grm_ = grm;
...
bool RuleCascade<Arc>::Init(const AbstractGrmManager<Arc>* grm,
                            std::vector<RuleTriple> rule_triples) {
  if (const absl::Status status = InitWithStatus(grm, rule_triples);
      !status.ok()) {
    LOG(ERROR) << status;
    return false;
  }
...
template <typename Arc>
absl::Status RuleCascade<Arc>::InitFromDefsWithStatus(
    const AbstractGrmManager<Arc>& grm, absl::Span<const std::string> rules) {
  grm_ = &grm;
...
bool RuleCascade<Arc>::InitFromDefs(const AbstractGrmManager<Arc>* grm,
                                    absl::Span<const std::string> rules) {
  if (const absl::Status status = InitFromDefsWithStatus(*grm, rules);
      !status.ok()) {
    LOG(ERROR) << status;
    return false;
  }
...
#endif  // OPENGRM_THRAX_ABSTRACT_GRM_MANAGER_H_
