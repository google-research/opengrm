...
static absl::btree_set<std::string> InitStaticKeywords() {
  absl::btree_set<std::string> keywords;
  keywords.insert("as");
...
const absl::btree_set<std::string> Lexer::kKeywords = InitStaticKeywords();
  ...
  while (!grammar_.empty() && !found_token && c != 0) {
    if (isspace(c)) {  // skip space
      ...
      while (true) {
        char curr_c = GetChar();
        if (curr_c == '\0') {
          LOG(ERROR) << "Found EOF without terminating string: "
        ...
        if (curr_c == '\\') {
          curr_c = GetChar();
          CHECK_NE(curr_c, '\0');
          if (curr_c != terminator) curr_token_.token_string += '\\';
      ...
      LOG(ERROR) << "Cannot parse character " << c << " ("
