...
#include "opengrm/thrax/rewrite-tester.h"
...
ABSL_FLAG(std::string, history_file, HISTORY_FILE, "Location of history file");
ABSL_FLAG(int64_t, noutput, 1,
...
void RewriteTester::Initialize() {
  ...
  for (size_t i = 0; i < rules_.size(); ++i) {
    ...
    if (vfst.InputSymbols()) {
      if (!byte_symtab_ && vfst.InputSymbols()->Name() ==
...
inline void InitializeHistoryFile() {
  if (absl::GetFlag(FLAGS_history_file).empty()) {
...
bool RewriteTester::ReadInput(std::string* s) {
  ...
  s->assign(input);
  if (!absl::GetFlag(FLAGS_history_file).empty()) add_history(input);
  free(input);
  if (!absl::GetFlag(FLAGS_history_file).empty())
    write_history(absl::GetFlag(FLAGS_history_file).c_str());
  return true;
}
#else   // HAVE_READLINE
bool RewriteTester::ReadInput(std::string* s) {
...
bool RewriteTester::AppendLabel(Label label, std::string* path) const {
  if (label != 0) {
    ...
    if (generated_symtab_ && !generated_symtab_->Find(label).empty()) {
      ...
      // space-separated.
      if (!path->empty()) *path += absl::GetFlag(FLAGS_fst_field_separator);
      *path += sym;
