...
#ifndef OPENGRM_OPERATORS_CDREWRITE_H_
#define OPENGRM_OPERATORS_CDREWRITE_H_
...
class CDRewriteRule {
 ...
 private:
  ...
  void MakeMarker(VectorFst<StdArc>* fst, const VectorFst<StdArc>& sigma,
                  MarkerType type,
                  absl::Span<const std::pair<Label, Label>> markers);
...
void CDRewriteRule<Arc>::IgnoreMarkers(
    MutableFst<Arc>* fst, absl::Span<const std::pair<Label, Label>> markers) {
...
void CDRewriteRule<Arc>::AddMarkersToSigma(
    MutableFst<Arc>* sigma, absl::Span<const std::pair<Label, Label>> markers) {
...
// alphabet (sigma).
template <class Arc>
inline void PrependSigmaStar(MutableFst<Arc>* fst, const Fst<Arc>& sigma) {
  Concat(sigma, fst);
  RmEpsilon(fst);
}
...
void CDRewriteRule<Arc>::Compile(const Fst<Arc>& sigma, MutableFst<Arc>* fst,
  ...
  if (add_initial_boundary_marker) {
    AddMarkersToSigma(&mutable_sigma,
                      {{initial_boundary_marker_, initial_boundary_marker_}});
  }
  if (add_final_boundary_marker) {
    AddMarkersToSigma(&mutable_sigma,
                      {{final_boundary_marker_, final_boundary_marker_}});
  }
  ...
  if (add_initial_boundary_marker || add_final_boundary_marker) {
    ...
    BoundaryInserter(sigma, &inserter, add_initial_boundary_marker,
                     add_final_boundary_marker);
    VectorFst<Arc> deleter;
    BoundaryDeleter(sigma, &deleter, add_initial_boundary_marker,
                    add_final_boundary_marker);
    VectorFst<Arc> tmp;
    ArcSort(fst, icomp);
...
void CDRewriteRule<Arc>::HandleBoundaryMarkers(const Fst<Arc>& sigma,
  ...
  if (del && (add_initial_boundary_marker || add_final_boundary_marker)) {
    ...
    for (StateIterator<VectorFst<Arc>> siter(del_sigma); !siter.Done();
         siter.Next()) {
...
void CDRewriteRule<Arc>::BoundaryInserter(const Fst<Arc>& sigma,
                                          ...
                                          bool add_initial_boundary_marker,
                                          bool add_final_boundary_marker) {
  HandleBoundaryMarkers(sigma, final_fst, false, add_initial_boundary_marker,
                        add_final_boundary_marker);
  Optimize(final_fst);
...
void CDRewriteRule<Arc>::BoundaryDeleter(const Fst<Arc>& sigma,
                                         ...
                                         bool add_initial_boundary_marker,
                                         bool add_final_boundary_marker) {
  HandleBoundaryMarkers(sigma, final_fst, true, add_initial_boundary_marker,
                        add_final_boundary_marker);
  Optimize(final_fst);
...
bool CDRewriteRule<Arc>::HasArcWithLabel(const Fst<Arc>& fst, Label label) {
  ...
  for (StateIterator<Fst<Arc>> siter(fst); !siter.Done(); siter.Next()) {
    for (ArcIterator<Fst<Arc>> aiter(fst, siter.Value()); !aiter.Done();
         aiter.Next()) {
      Arc arc = aiter.Value();
      if (arc.ilabel == label || arc.olabel == label) return true;
    }
  }
  return false;
}
...
#endif  // OPENGRM_OPERATORS_CDREWRITE_H_
