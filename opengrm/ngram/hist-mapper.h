...
#ifndef OPENGRM_NGRAM_HIST_MAPPER_H_
#define OPENGRM_NGRAM_HIST_MAPPER_H_
...
class ToHistogramMapper {
 public:
  ...
  using Weight = fst::PowerWeight<fst::TropicalWeight, ::ngram::kHistogramBins>;
  ...
  ToArc operator()(const Arc& arc) const {
    ...
    weights[0] = fst::TropicalWeight(val);
    ...
    if (index < weights.size() - 1) {
      weights[index + 1] = fst::TropicalWeight(-log(exp(-val) - round_down));
    }
    if (index && index < weights.size()) {
      weights[index] = fst::TropicalWeight(-log(round_up - exp(-val)));
    }
...
#endif  // OPENGRM_NGRAM_HIST_MAPPER_H_
