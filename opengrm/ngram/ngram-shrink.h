...
#ifndef OPENGRM_NGRAM_NGRAM_SHRINK_H_
#define OPENGRM_NGRAM_NGRAM_SHRINK_H_
 ...
 private:
  ...
  void InitStateLabelData() {
    ...
    for (StateId st = 0; st < ns_; ++st) {
      ...
      // below.
      if (GetExpandedFst().Final(st) != Arc::Weight::Zero()) {
        ++num_arcs;
      }
    }
    ...
    for (StateId st = 0; st < ns_; ++st) {
      ...
      for (fst::ArcIterator<fst::ExpandedFst<Arc>> aiter(GetExpandedFst(), st);
  ...
  typename std::vector<LabelData>::iterator FindStateAndLabel(
    ...
    auto it = std::lower_bound(begin, end, LabelData(label),
                               [](const LabelData& a, const LabelData& b) {
                                 return a.label < b.label;
                               });
  ...
  size_t PointPrunedArcs(absl::Span<const ShrinkArcStats> shrink_arcs,
                         StateId st);
...
double NGramShrink<Arc>::GetShrinkScore(const ShrinkArcStats& arc, StateId st,
  ...
  if (calc_score) {  // Calculates local score and compares with maximum.
    shrink_score = UpdateScore(st, label, ShrinkScore(shrink_state_[st], arc));
...
size_t NGramShrink<Arc>::FillShrinkArcInfo(
  ...
  fst::Matcher<fst::Fst<Arc>> matcher(GetFst(),
                                      fst::MATCH_INPUT);  // to find backoff
  ...
  for (fst::ArcIterator<fst::ExpandedFst<Arc>> aiter(GetExpandedFst(), st);
    ...
    if (arc.ilabel == BackoffLabel()) {
      // placeholder
      shrink_arcs->push_back(ShrinkArcStats(0, 0, arc.ilabel, fst::kNoStateId,
                                            fst::kNoStateId, true));
...
size_t NGramShrink<Arc>::PointPrunedArcs(
    absl::Span<const ShrinkArcStats> shrink_arcs, StateId st) {
  size_t acnt = 0;
  size_t pruned_cnt = 0;
  for (fst::MutableArcIterator<fst::MutableFst<Arc>> aiter(GetMutableFst(), st);
       !aiter.Done(); aiter.Next()) {
    Arc arc = aiter.Value();
...
void NGramShrink<Arc>::PointArcsAwayFromDead() {
  for (StateId st = 0; st < ns_; ++st) {
    ...
    for (fst::MutableArcIterator<fst::MutableFst<Arc>> aiter(GetMutableFst(),
                                                             st);
         !aiter.Done(); aiter.Next()) {
      Arc arc = aiter.Value();
...
void NGramShrink<Arc>::PointDeadBackoffArcs() {
  for (StateId st = 0; st < ns_; ++st) {
    ...
    fst::MutableArcIterator<fst::MutableFst<Arc>> aiter(GetMutableFst(), st);
...
bool NGramShrinkModel(
    ...
    absl::string_view context_pattern = "", int shrink_opt = 0,
...
bool NGramShrinkModel(fst::StdMutableFst* fst, const std::string& method,
                      ...
                      int shrink_opt = 0, fst::StdArc::Label backoff_label = 0,
                      double norm_eps = kNormEps,
                      bool check_consistency = false);

}  // namespace ngram

#endif  // OPENGRM_NGRAM_NGRAM_SHRINK_H_
