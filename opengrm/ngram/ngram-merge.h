...
#ifndef OPENGRM_NGRAM_NGRAM_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_MERGE_H_
...
class NGramMerge : public NGramMutableModel<Arc> {
 ...
 private:
  ...
  bool MergeWordLists() {
    ...
    absl::btree_map<int64_t, int64_t>
  ...
  void SetupMergeMaps() {
    ...
    } else if (unigram1 != fst::kNoStateId) {  // merging unigram into order > 1
  ...
  void MergeBackedOffToMap() {
    for (StateId st = 0; st < ngram1_ns_; ++st) {
      ...
      if (bo < 0) continue;
      for (fst::ArcIterator<fst::Fst<Arc>> aiter(GetFst(), st); !aiter.Done();
           aiter.Next()) {
  ...
  void MergeFsts(int max_order) {
    ...
    absl::btree_set<Label> shared;  // shared arcs between st and ist
  ...
  void MergeUnsharedArcs1(StateId st, StateId ist,
                          const absl::btree_set<Label>& shared) {
  ...
  void MergeUnsharedArcs2(StateId st, StateId ist,
    ...
    if (shared.count(fst::kNoLabel) == 0) {  // not found
      ...
      if (ScalarValue(final2) != ScalarValue(Weight::Zero())) {
        ...
        Weight merge_final =
            MergeWeights(bst, ist, fst::kNoLabel, cost, final2, false, true);
        GetMutableFst()->SetFinal(st, merge_final);
      }
    }
  }
  ...
  void MergeDests1(StateId low_src, Label label, StateId old_dest,
    ...
    auto range = backed_off_to_.equal_range(pr);
  ...
  StateId MergeBackoffDest(StateId st, Label label, bool from1, Weight* cost) {
    ...
    fst::Matcher<fst::Fst<Arc>> matcher(ngram->GetFst(), fst::MATCH_INPUT);
    matcher.SetState(st);
...
#endif  // OPENGRM_NGRAM_NGRAM_MERGE_H_
