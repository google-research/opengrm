...
#include "opengrm/ngram/ngram-context-prune.h"
#include "opengrm/ngram/ngram-count-prune.h"
...
void CheckShrinkOptions(absl::string_view method, int64_t target_num,
...
bool NGramShrinkModel(fst::StdMutableFst* fst, const std::string& method,
                      ...
                      absl::string_view context_pattern, int shrink_opt,
                      fst::StdArc::Label backoff_label, double norm_eps,
                      bool check_consistency) {
  return NGramShrinkModel(
      fst, method, std::set<std::vector<fst::StdArc::Label>>(), tot_uni, theta,
      target_num, min_order, count_pattern, context_pattern, shrink_opt,
...
bool NGramShrinkModel(
    ...
    const std::set<std::vector<fst::StdArc::Label>>& ngram_list, double tot_uni,
    double theta, int64_t target_num, int32_t min_order,
    const std::string& count_pattern, absl::string_view context_pattern,
  ...
  if (method == "list_prune") {
    ...
    if (full_context) {
      NGramSeymoreShrink ngramsh(fst, theta, shrink_opt, tot_uni, backoff_label,
                                 norm_eps, check_consistency);
      if (target_num >= 0) {
        ngramsh.CalculateTheta(target_num, min_order);
        if (ngramsh.Error()) return false;
