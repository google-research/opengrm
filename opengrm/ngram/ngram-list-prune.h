...
#ifndef OPENGRM_NGRAM_NGRAM_LIST_PRUNE_H_
#define OPENGRM_NGRAM_NGRAM_LIST_PRUNE_H_
...
class NGramListPrune : public NGramShrink<fst::StdArc> {
 ...
 private:
  ...
  // TODO: unify with struct in ngram-count.h and benchmark.
  struct PairHash {
  ...
  void InitializeStateVectors() {
    ...
    fst::Matcher<fst::Fst<fst::StdArc>> matcher(GetFst(), fst::MATCH_INPUT);
  ...
  bool CollectSuffixInfo() {
    ...
    for (int order = 1; order <= HiOrder(); ++order) {
      for (auto s : order_states[order]) {
        fst::ArcIterator<fst::ExpandedFst<fst::StdArc>> aiter(GetExpandedFst(),
                                                              s);
  ...
  absl::flat_hash_set<StateId> highest_order_prune_origin_state_;
  absl::flat_hash_set<std::pair<StateId, Label>, PairHash>
...
void GetNGramListToPrune(absl::Span<const std::string> ngrams_to_prune,
...
#endif  // OPENGRM_NGRAM_NGRAM_LIST_PRUNE_H_
