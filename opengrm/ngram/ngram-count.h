...
#ifndef OPENGRM_NGRAM_NGRAM_COUNT_H_
#define OPENGRM_NGRAM_NGRAM_COUNT_H_
 ...
 public:
  ...
  bool Count(fst::MutableFst<Arc>* fst) {
    ...
    if (fst->Properties(fst::kTopSorted, true) != fst::kTopSorted) {
      if (!fst::TopSort(fst)) {
        // TODO: support key in error message.
  ...
  void GetFst(fst::MutableFst<Arc>* fst) {
    ...
    for (size_t s = 0; s < states_.size(); ++s) {
      ...
      if (states_[s].backoff_state != -1)
        fst->AddArc(s,
                    Arc(0, 0, Arc::Weight::Zero(), states_[s].backoff_state));
    }
  ...
  ssize_t NGramNextState(ssize_t arc_id) const {
 ...
 private:
  ...
  struct PairHash {
    size_t operator()(const Pair& p) const {
      ...
      // TODO: run benchmark using Compose's hash function
      // return static_cast<size_t>(p.first + p.second * 7853);
    }
  };

  // TODO: run benchmark using map instead of unordered map
...
template <class Arc>
bool NGramCounter<Weight, Label>::CountFromStringFst(const fst::Fst<Arc>& fst) {
...
template <class Arc>
void GetNGramCountOfCounts(const fst::Fst<Arc>& fst, fst::StdMutableFst* ccfst,
                           int in_order, absl::string_view context_pattern) {
...
#endif  // OPENGRM_NGRAM_NGRAM_COUNT_H_
