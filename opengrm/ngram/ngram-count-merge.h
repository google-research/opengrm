...
#ifndef OPENGRM_NGRAM_NGRAM_COUNT_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_COUNT_MERGE_H_
...
class NGramCountMerge : public NGramMerge<fst::StdArc> {
 public:
  ...
  explicit NGramCountMerge(fst::StdMutableFst* infst1, Label backoff_label = 0,
 ...
 protected:
  ...
  Weight MergeWeights(StateId s1, StateId s2, Label Label, Weight w1, Weight w2,
    ...
    if (in_fst1 && in_fst2) {
      return Weight(NegLogSum(w1.Value() + alpha_, w2.Value() + beta_));
    } else if (in_fst1) {
      return Weight(w1.Value() + alpha_);
    } else {
      return Weight(w2.Value() + beta_);
    }
  }
...
#endif  // OPENGRM_NGRAM_NGRAM_COUNT_MERGE_H_
