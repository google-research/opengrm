...
#ifndef OPENGRM_NGRAM_NGRAM_CONTEXT_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_CONTEXT_MERGE_H_
...
class NGramContextMerge : public NGramMerge<fst::StdArc> {
 public:
  ...
  void MergeNGramModels(const fst::StdFst& infst2,
                        absl::string_view context_pattern, bool norm = false) {
    context_ =
 ...
 protected:
  ...
  Weight MergeWeights(StateId s1, StateId s2, Label label, Weight w1, Weight w2,
    ...
    if (in_fst1 && in_fst2) {
      ...
      return context_->HasContext(ngram, false) ? w2 : w1;
    } else if (in_fst1) {
      return w1;
    } else {
      return w2;
...
#endif  // OPENGRM_NGRAM_NGRAM_CONTEXT_MERGE_H_
