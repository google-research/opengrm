...
bool NGramMarginal::CalculateStateProbs() {
  ...
  if (!NGramModel::CalculateStateProbs(&weights, false)) return false;
  DiffProbs(&weights);
...
void NGramMarginal::DiffProbs(std::vector<double>* weights) const {
  ...
  for (size_t st = 0; st < weights->size(); ++st) {
    if ((*weights)[st] < kEffectiveZero) (*weights)[st] = kEffectiveZero;
    sum += (*weights)[st];
  }
  // Normalizes.
  for (size_t st = 0; st < weights->size(); ++st) (*weights)[st] /= sum;
}
...
void NGramMarginal::SetSaneArcWeights(StateId st, std::vector<double>* wts,
  ...
  GetMutableFst()->SetFinal(
      st, fst::TropicalWeight((*wts)[idx]));  // then set value
  for (MutableArcIterator<StdMutableFst> aiter(GetMutableFst(), st);
    ...
    (*wts)[idx] -= norm;
    arc.weight = fst::TropicalWeight((*wts)[idx]);
    aiter.SetValue(arc);
  }
}
...
void NGramMarginal::CalculateNewStateWeights(StateId st) {
  ...
  while (need_upd && (upd_count == 0 || upd_count < max_bo_updates_)) {
    ...
    need_upd = (max_bo_updates_ > 0) ? HigherOrderBackoffRecalc(st, &wts, &norm)
                                     : false;
    ++upd_count;
  }
}
