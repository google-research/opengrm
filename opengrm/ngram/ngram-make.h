...
#ifndef OPENGRM_NGRAM_NGRAM_MAKE_H_
#define OPENGRM_NGRAM_NGRAM_MAKE_H_
...
class NGramMake : public NGramMutableModel<Arc> {
 public:
  ...
  NGramMake(fst::MutableFst<Arc>* infst, bool backoff, Label backoff_label = 0,
 ...
 private:
  ...
  bool HasAllArcsInBackoff(StateId st) const {
  ...
  void NormalizeStateArcs(StateId st, double norm, double neglog_bo_prob,
                          absl::Span<const double> discounts) {
    StateId bo = GetBackoff(st, nullptr);
...
bool NGramMakeModel(fst::StdMutableFst* fst, const std::string& method,
                    const fst::StdFst* ccfst = nullptr, bool backoff = false,
...
// TODO: Figure out how to make this compatible with scripting interface.

}  // namespace ngram

#endif  // OPENGRM_NGRAM_NGRAM_MAKE_H_
