...
#ifndef OPENGRM_NGRAM_NGRAM_TRANSFER_H_
#define OPENGRM_NGRAM_NGRAM_TRANSFER_H_
...
class NGramTransfer {
 public:
  ...
  NGramTransfer(fst::MutableFst<Arc>* fst, absl::string_view context_pattern,
  ...
  NGramTransfer(const fst::Fst<Arc>& fst, absl::string_view context_pattern,
                Label backoff_label = 0, double norm_eps = kNormEps)
 ...
 private:
  ...
  void InitSrc(const fst::Fst<Arc>& fst, absl::string_view context_pattern,
               Label backoff_label, double norm_eps) {
    src_fst_.reset(fst.Copy());
    src_matcher_.reset(
        new fst::Matcher<fst::Fst<Arc>>(*src_fst_, fst::MATCH_INPUT));
    src_model_.reset(
  ...
  void InitDest(fst::MutableFst<Arc>* fst, absl::string_view context_pattern,
                Label backoff_label, double norm_eps) {
    dest_fst_ = fst;
    dest_model_.reset(
...
typename Arc::StateId NGramTransfer<Arc>::FindNextState(StateId s,
  ...
  Label find_backoff_label =
      src_model_->BackoffLabel() ? src_model_->BackoffLabel() : fst::kNoLabel;
  bool found = false;
  do {
...
bool NGramTransfer<Arc>::TransferNGrams() const {
  ...
  for (int order = 1; order <= src_model_->HiOrder(); ++order) {
    for (StateId s = 0; s < src_model_->NumStates(); ++s) {
      ...
      for (fst::ArcIterator<fst::Fst<Arc>> aiter(*dest_fst_, sp); !aiter.Done();
           aiter.Next()) {
      ...
      for (fst::ArcIterator<fst::Fst<Arc>> aiter(*src_fst_, s); !aiter.Done();
           aiter.Next()) {
        const Arc& arc = aiter.Value();
      ...
      if (NGramModel<Arc>::ScalarValue(src_fst_->Final(s)) !=
          ...
          (add_missing || NGramModel<Arc>::ScalarValue(dest_fst_->Final(sp)) !=
                              NGramModel<Arc>::ScalarValue(Weight::Zero()))) {
...
#endif  // OPENGRM_NGRAM_NGRAM_TRANSFER_H_
