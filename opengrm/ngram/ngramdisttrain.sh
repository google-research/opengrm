...
if [[ -z ${IFILE} || -z ${OFILE} || -z ${ITYPE} || -z ${OTYPE} \
 ...
 --itype <input type> --otype <output type>"
  ...
  echo "  --bins                number of bins for Katz or absolute discounting"
...
# Transfers from sub-context shards; argument(s) are additional options to
# ngramtransfer.
ngram_transfer_to() {
...
# Completes each context shard; argument(s) are additional options to the
# (final) ngramtransfer.
ngram_complete() {
  ngram_sub_split
  ngram_transfer_to
...
# Smoothes model; argument(s) are additional options to ngrammake.
ngram_make() {
  message "Smoothing model"
  if [[ -n ${CONTEXTS} && ${SMOOTH_METHOD} = kneser_ney ]]; then
  ...
  update
  ITYPE=lm
  if [[ -n ${CONTEXTS} && ${SMOOTH_METHOD} = witten_bell ]]; then
