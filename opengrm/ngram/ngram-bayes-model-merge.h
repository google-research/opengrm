...
#ifndef OPENGRM_NGRAM_NGRAM_BAYES_MODEL_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_BAYES_MODEL_MERGE_H_
 ...
 protected:
  ...
  fst::StdArc::Weight MergeWeights(StateId s1, StateId s2, Label label,
    ...
    if (label == BackoffLabel()) {  // don't modify (needed) backoff weights
      ...
      return fst::StdArc::Weight(
          NegLogSum(w1.Value() + alpha, w2.Value() + beta));
    }
  }
...
#endif  // OPENGRM_NGRAM_NGRAM_BAYES_MODEL_MERGE_H_
