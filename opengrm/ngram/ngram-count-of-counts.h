...
#ifndef OPENGRM_NGRAM_NGRAM_COUNT_OF_COUNTS_H_
#define OPENGRM_NGRAM_NGRAM_COUNT_OF_COUNTS_H_
...
class NGramCountOfCounts {
 public:
  ...
  NGramCountOfCounts(absl::string_view context_pattern, int order,
                     int bins = -1)
      : bins_(bins <= 0 || bins > kMaxBins ? kMaxBins : bins),
  ...
  NGramCountOfCounts(absl::Span<const Label> context_begin,
                     absl::Span<const Label> context_end, int order,
                     int bins = -1)
      : bins_(bins <= 0 || bins > kMaxBins ? kMaxBins : bins),
  ...
  void ShowCounts(absl::Span<const std::vector<double>> hist,
  ...
  void ShowCounts(absl::string_view label) const {
  ...
  void GetFst(fst::StdMutableFst* fst) const {
    ...
    fst->SetFinal(s, StdArc::Weight(-log(sum)));
...
#endif  // OPENGRM_NGRAM_NGRAM_COUNT_OF_COUNTS_H_
