...
#ifndef OPENGRM_NGRAM_NGRAM_MUTABLE_MODEL_H_
#define OPENGRM_NGRAM_NGRAM_MUTABLE_MODEL_H_
...
class NGramMutableModel : public NGramModel<Arc> {
 public:
  ...
  explicit NGramMutableModel(fst::MutableFst<Arc>* infst, Label backoff_label,
  ...
  const fst::ExpandedFst<Arc>& GetExpandedFst() const { return *mutable_fst_; }
  ...
  void SortStates() {
    ...
    fst::StateSort(mutable_fst_, inv_order);
  }
  ...
  Weight ScaleWeight(Weight w, double scalar) const;
 ...
 protected:
  ...
  bool FindMutableArc(fst::MutableArcIterator<fst::MutableFst<Arc>>* biter,
                      Label label) const {
  ...
  void ScaleStateWeight(StateId st, double scale) {
    ...
    for (fst::MutableArcIterator<fst::MutableFst<Arc>> aiter(mutable_fst_, st);
  ...
  void DeBackoffNGramModel() {
    for (StateId st = 0; st < mutable_fst_->NumStates(); ++st) {
      double hi_neglog_sum, low_neglog_sum;
      if (CalcBONegLogSums(st, &hi_neglog_sum, &low_neglog_sum)) {
        fst::MutableArcIterator<fst::MutableFst<Arc>> aiter(mutable_fst_, st);
...
void NGramMutableModel<Arc>::SetScalarValue(
  ...
  *w = typename Arc::Weight(scalar);
}

template <>
inline void NGramMutableModel<HistogramArc>::SetScalarValue(
    NGramMutableModel<HistogramArc>::Weight* w, double scalar) {
  w->SetValue(0, fst::TropicalWeightTpl<float>(scalar));
}

template <typename Arc>
typename NGramMutableModel<Arc>::Weight NGramMutableModel<Arc>::ScaleWeight(
    NGramMutableModel<Arc>::Weight w, double scalar) const {
  return Times(fst::TropicalWeightTpl<float>(scalar), w);
}

template <>
inline NGramMutableModel<HistogramArc>::Weight
NGramMutableModel<HistogramArc>::ScaleWeight(
    NGramMutableModel<HistogramArc>::Weight w, double scalar) const {
  w.SetValue(0, Times(w.Value(0), fst::TropicalWeightTpl<float>(scalar)));
  return w;
}

}  // namespace ngram

#endif  // OPENGRM_NGRAM_NGRAM_MUTABLE_MODEL_H_
