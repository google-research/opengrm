...
#ifndef OPENGRM_NGRAM_NGRAM_MODEL_H_
#define OPENGRM_NGRAM_NGRAM_MODEL_H_
...
inline constexpr double kNormEps = 0.001;
inline constexpr double kFloatEps = 0.000001;
inline constexpr double kInfBackoff = 99.00;

// Calculate - log( exp(a - b) + 1 ) for use in high precision NegLogSum
inline double NegLogDeltaValue(double a, double b, double* c) {
  const double x = exp(a - b);
  double delta = -log(x + 1);
...
class NGramModel {
 public:
  ...
  NGramModel(const fst::Fst<Arc>& infst, Label backoff_label, double norm_eps,
             bool state_ngrams)
      : fst_(infst),
  ...
  // 'word' in the ngram.
  StateId NGramState(absl::Span<const Label> ngram) const {
  ...
  int NumNGrams(StateId st) const {
 ...
 protected:
  ...
  void FillStateCounts(std::vector<double>* state_counts) const {
...
inline typename HistogramArc::Weight NGramModel<HistogramArc>::UnitCount() {
  ...
  static const fst::PowerWeight<fst::StdArc::Weight, kHistogramBins> one(
...
#endif  // OPENGRM_NGRAM_NGRAM_MODEL_H_
