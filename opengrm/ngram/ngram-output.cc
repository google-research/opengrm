...
ABSL_FLAG(std::string, start_symbol, "<s>", "Class label for sentence start");
ABSL_FLAG(std::string, end_symbol, "</s>", "Class label for sentence end");
...
void NGramOutput::ShowNGramModel(NGramOutput::ShowBackoff showeps, bool neglogs,
  ...
  if (ARPA) {
    ...
    if (UnigramState() >= 0) {  // show n-grams from unigram state
      ...
      str = absl::GetFlag(FLAGS_start_symbol);  // init n-grams from <s> state
    } else {
      start_wt =
    ...
    if (InContext(GetFst().Start())) {
      ostrm_ << absl::GetFlag(FLAGS_start_symbol) << '\t' << start_wt;
      if (showeps == ShowBackoff::INLINE &&
          UnigramState() >= 0)  // <s> state exists, then show backoff
        ostrm_ << '\t'
               << WeightRep(GetBackoffCost(GetFst().Start()).Value(), neglogs,
                            intcnts);
      ostrm_ << '\n';
    }
...
bool NGramOutput::PerplexityNGramModel(
    absl::Span<const std::unique_ptr<fst::StdVectorFst>> infsts, int32_t v,
...
void NGramOutput::ShowARPANGrams(StdArc::StateId st, absl::string_view str,
                                 int order) const {
  ...
  if (show && order == StateOrder(st) &&               // only show target order
    ...
    ostrm_ << absl::GetFlag(FLAGS_end_symbol) << '\n';
  }
  for (ArcIterator<StdExpandedFst> aiter(GetExpandedFst(), st); !aiter.Done();
    ...
    if (show && order == StateOrder(st)) {      // only show target order
      ...
      if (StateOrder(arc.nextstate) > StateOrder(st))  // show backoff
        ostrm_ << "\t"
               << ShowLogNewBase(ScalarValue(GetBackoffCost(arc.nextstate)),
                                 10);
      ostrm_ << '\n';
    }
...
void NGramOutput::ShowARPAModel() const {
  ...
  for (int i = 0; i < HiOrder(); ++i) {
    ...
    if (i == 0 &&  // following SRILM, add <s> unigram w/ dummy weight of -99
      ...
      ostrm_ << "-99\t" << absl::GetFlag(FLAGS_start_symbol) << '\t';
    ...
    if (UnigramState() >= 0) {
      ...
      ShowARPANGrams(GetFst().Start(), absl::GetFlag(FLAGS_start_symbol),
                     i + 1);
...
void NGramOutput::ShowNGrams(StdArc::StateId st, absl::string_view str,
  ...
  for (ArcIterator<StdExpandedFst> aiter(GetExpandedFst(), st); !aiter.Done();
    ...
    if (show) {
      ...
      if (showeps == ShowBackoff::INLINE &&
        ...
        ostrm_ << "\t"
               << WeightRep(GetBackoffCost(arc.nextstate).Value(), neglogs,
                            intcnts);
      ostrm_ << '\n';
    }
    if (arc.ilabel != BackoffLabel() &&  // depth-first traversal
...
void NGramOutput::ShowPhiPerplexity(const ComposeFst<StdArc>& cfst,
  ...
  std::string history = absl::GetFlag(FLAGS_start_symbol) + " ";
  ...
  if (verbose)
    ShowNGramProb(absl::GetFlag(FLAGS_end_symbol), history, false, -1,
                  -ngram_cost);
  if (InContext(st)) neglogprob += ngram_cost;
...
void NGramOutput::ApplyFinalCost(StateId mst, std::string history, int word_cnt,
  ...
  if (verbose) {
    ShowNGramProb(absl::GetFlag(FLAGS_end_symbol), history, (order < 0), order,
                  ngram_cost);
...
void NGramOutput::RandNGramModel(absl::BitGenRef bit_gen, int64_t samples,
                                 bool show_backoff) const {
...
void NGramOutput::RenormUnigramForOOV(Label special_label, Label OOV_label,
  ...
  if (OOV_label < 0 && OOV_probability > 0.0) {        // default OOV with prob
    StdArc::Weight renorm(-log(1 - OOV_probability));  // renorm constant
    ...
    // spread class prob around members
    StdArc::Weight renorm(log(OOV_class_size));
