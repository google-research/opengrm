...
void ReadTokenString(absl::string_view str, std::vector<std::string>* words) {
  auto it = str.cbegin();
  while (it < str.cend()) {
    if (words->empty() || absl::ascii_isspace(*it)) {
      while (absl::ascii_isspace(*it)) ++it;       // Skips whitespace sequence.
...
NGramInput::NGramInput(std::istream& istrm, std::ostream& ostrm,
                       ...
                       absl::string_view oov_symbol,
                       absl::string_view start_symbol,
                       absl::string_view end_symbol)
...
bool NGramInput::GetWhiteSpaceToken(std::string::const_iterator* it,
  ...
  while (absl::ascii_isspace(**it)) ++*it;
  // No further tokens to be found in string.
  if (*it == str.cend()) return false;
  while (*it < str.cend() && !absl::ascii_isspace(**it)) {
    *token += **it;
    ++*it;
  }
  return true;
...
typename NGramInput::StateId NGramInput::GetHistoryState(
    absl::Span<const std::string> words, absl::Span<const Label> last_labels,
    absl::Span<const StateId> last_states, StateId st) {
...
bool NGramInput::ARPAHeaderStringMatch(absl::string_view tomatch) {
  std::string str;
...
void NGramInput::CheckInfVal(absl::string_view token, double* val) {
...
typename NGramInput::StateId NGramInput::GetLabelNextStateNoFail(
    StateId st, Label label) const {
  Matcher<MutableFst<Arc>> matcher(*fst_, MATCH_INPUT);
  matcher.SetState(st);
  if (matcher.Find(label)) {
    return matcher.Value().nextstate;
...
void NGramInput::AddNGramArc(StateId st, StateId nextstate, Label label,
  ...
  if (endsym) {  // </s> requires no arc, just final cost.
    fst_->SetFinal(st, StdArc::Weight(ngram_log_prob));
...
void NGramInput::ReadARPAOrder(std::vector<int>* orders, int order,
  ...
  for (auto i = 0; i < (*orders)[order]; ++i) {
    ...
    if (Error()) return;
    if (endsym) {
      ngram_counter->SetFinalNGramWeight(st, Log64Weight(nlprob));
    } else if (!stsym) {
      ...
      ngram_counter->SetNGramWeight(arc_id, Log64Weight(nlprob));
...
typename NGramInput::StateId NGramInput::FindNewDest(StateId st) const {
  StateId newdest = st;
...
void NGramInput::SetARPABackoffWeights(absl::Span<const double> boweights) {
    ...
    if (st < boweights.size()) {
      ...
      if (arc.ilabel == 0 || boprob != StdArc::Weight::Zero().Value()) {
        if (arc.ilabel != 0) {
          ...
          arc.weight = StdArc::Weight(boprob);
        }
        aiter.SetValue(arc);
      }
    }
...
double NGramInput::GetFinalBackoff(StateId st) const {
  if (fst_->Final(st) != StdArc::Weight::Zero()) return fst_->Final(st).Value();
  double bocost;
  auto bostate = GetBackoffAndCost(st, &bocost);
  if (bostate >= 0)
    fst_->SetFinal(st, StdArc::Weight(bocost + GetFinalBackoff(bostate)));
    ...
    for (MutableArcIterator<StdMutableFst> aiter(fst_.get(), st); !aiter.Done();
      ...
      if (arc.ilabel == 0) {
        ...
        if (arc.weight == StdArc::Weight::Zero()) {
          arc.weight =
              StdArc::Weight(boprob + GetLowerOrderProb(bostate, arc.ilabel));
          if (Error()) return;
          aiter.SetValue(arc);
        }
      }
...
void NGramInput::CheckInitState(absl::Span<const std::string> words,
...
int NGramInput::UpdateLast(absl::Span<const std::string> words,
...
double NGramInput::FillStringLabels(const std::string& str,
  ...
  if (string_counts) {
    ...
    if (!absl::SimpleAtod(token, &count)) SetError();
    token.clear();
  }
...
void NGramInput::DumpFst(bool incl_symbols, bool output) const {
  if (incl_symbols) {
