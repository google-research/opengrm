...
#ifndef OPENGRM_NGRAM_NGRAM_HIST_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_HIST_MERGE_H_
...
#include "opengrm/ngram/ngram-merge.h"
...
class NGramHistMerge : public NGramMerge<HistogramArc> {
 ...
 private:
  ...
  Weight WeightSum(Weight w1, Weight w2) const {
    ...
    v[0] = fst::TropicalWeight(
        NegLogSum(w1.Value(0).Value(), w2.Value(0).Value()));

    for (int k = 0; k < v.size() - 1; k++) {
      for (int j = 0; j <= k; j++) {
        v[k + 1] = fst::TropicalWeight(
            NegLogSum(v[k + 1].Value(),
                      w1.Value(j + 1).Value() + w2.Value(k - j + 1).Value()));
      }
    }
    return fst::PowerWeight<fst::TropicalWeight, kHistogramBins>(v.begin(),
                                                                 v.end());
  }
...
#endif  // OPENGRM_NGRAM_NGRAM_HIST_MERGE_H_
