...
#ifndef OPENGRM_NGRAM_NGRAM_RANDGEN_H_
#define OPENGRM_NGRAM_NGRAM_RANDGEN_H_
...
class NGramArcSelector {
 public:
  ...
  explicit NGramArcSelector() = default;
  ...
  size_t operator()(absl::BitGenRef bit_gen, const fst::Fst<A>& fst, StateId s,
    ...
    double r = absl::Uniform<double>(bit_gen, 0.0, 1.0);
    ...
    return accumulator->LowerBound(typename A::Weight(-log(z)), &aiter);
  }
 ...
 public:
  ...
  bool Sample(absl::BitGenRef bit_gen, const RandState<A>& rstate) {
    ...
    for (size_t i = 0; i < rstate.nsamples; ++i) {
      ...
      do {
        pos = arc_selector_(bit_gen, fst_, rstate.state_id, total_prob,
                            accumulator_.get());
...
#endif  // OPENGRM_NGRAM_NGRAM_RANDGEN_H_
