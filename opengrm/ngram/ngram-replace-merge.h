...
#ifndef OPENGRM_NGRAM_NGRAM_REPLACE_MERGE_H_
#define OPENGRM_NGRAM_NGRAM_REPLACE_MERGE_H_
...
class NGramReplaceMerge : public NGramMerge<fst::StdArc> {
 public:
  ...
  void MergeNGramModels(const fst::StdFst& infst2, int max_replace_order = -1,
                        bool norm = false) {
    if (Error()) return;
    NGramModel<fst::StdArc> mod2(infst2);
    if (!NGramMerge<fst::StdArc>::MergeNGramModels(infst2, /* norm = */ false,
                                                   max_replace_order)) {
 ...
 private:
  ...
  void TrimTopology() {
    ...
    for (StateId st = 0; st < NumStates(); ++st) {
      ...
      for (fst::MutableArcIterator<fst::MutableFst<fst::StdArc>> aiter(
               GetMutableFst(), st);
...
#endif  // OPENGRM_NGRAM_NGRAM_REPLACE_MERGE_H_
