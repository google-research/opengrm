...
#ifndef OPENGRM_NGRAM_NGRAM_INPUT_H_
#define OPENGRM_NGRAM_NGRAM_INPUT_H_
...
class NGramInput {
 public:
  ...
  NGramInput(std::istream& istrm, std::ostream& ostrm,
             const std::string& symbols, absl::string_view epsilon_symbol,
             absl::string_view oov_symbol, absl::string_view start_symbol,
 ...
 private:
  ...
  StateId GetBackoffAndCost(StateId st, double* cost) const;

  // Just returns backoff state.
  StateId GetBackoff(StateId st) const {
    return GetBackoffAndCost(st, nullptr);
  }

  // Ensures matching with appropriate ARPA header strings.
  bool ARPAHeaderStringMatch(absl::string_view tomatch);
  ...
  bool GetStringVal(std::string::const_iterator* it, const std::string& str,
                    A* val, std::string* keeptoken) const {
    std::string token;
  ...
  static void CheckInfVal(absl::string_view token, double* val);
  ...
  StateId GetLabelNextStateNoFail(StateId st, Label label) const;
  ...
  void CheckInitState(absl::Span<const std::string> words, StateId* init,
  ...
  StateId GetHistoryState(absl::Span<const std::string> words,
                          absl::Span<const Label> last_labels,
  ...
  static int UpdateLast(absl::Span<const std::string> words, int longest_ngram,
  ...
  void DumpFst(bool incl_symbols, bool output) const;
...
#endif  // OPENGRM_NGRAM_NGRAM_INPUT_H_
