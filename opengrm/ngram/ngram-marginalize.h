...
#ifndef OPENGRM_NGRAM_NGRAM_MARGINALIZE_H_
#define OPENGRM_NGRAM_NGRAM_MARGINALIZE_H_
...
class NGramMarginal : public NGramMutableModel<fst::StdArc> {
 public:
  ...
  explicit NGramMarginal(fst::StdMutableFst* infst, Label backoff_label = 0,
      ...
      : NGramMutableModel<fst::StdArc>(infst, backoff_label, norm_eps,
 ...
 private:
  ...
  double MassReservedForBackoff(StateId st, double original_norm,
                                double new_norm) {
    if (st == UnigramState() || new_norm <= 0.0) {
      // Keeps original mass reserved for backoff.
      if (st != UnigramState()) {
        VLOG(2) << "NGramMarginalize: using original reserved mass: st: " << st;
      }
...
#endif  // OPENGRM_NGRAM_NGRAM_MARGINALIZE_H_
