...
#ifndef OPENGRM_SFST_BACKOFF_H_
#define OPENGRM_SFST_BACKOFF_H_
...
class Backoff {
 ...
 private:
  ...
  using PairArcMap = absl::node_hash_map<Pair, ssize_t, PairHash>;
...
Backoff<Arc>::Backoff(const fst::Fst<Arc>& fst, Label phi_label,
  ...
  if (phi_label_ == fst::kNoStateId) return;
  ...
  for (StateId i = top_order_.size() - 1; i >= 0; --i) {
    ...
    if (matcher.Find(phi_label_)) {
      for (; !matcher.Done(); matcher.Next()) {
        ...
        if (bo_state.backoff_state != fst::kNoStateId) {
...
bool SumBackoff(fst::MutableFst<Arc>* fst, typename Arc::Label phi_label) {
  ...
  for (StateId i = 0; i < fst->NumStates(); ++i) {
    ...
    if (bos == fst::kNoStateId) continue;
    ...
    fst::MutableArcIterator<fst::MutableFst<Arc>> miter(fst, bos);
...
bool DiffBackoff(fst::MutableFst<Arc>* fst, typename Arc::Label phi_label,
  ...
  for (StateId i = fst->NumStates() - 1; i >= 0; --i) {
    ...
    if (final != Weight::Zero()) {
      Weight bo_final = fst->Final(bos);
      fst::Log64Weight w1 = to_log(bo_final);
      fst::Log64Weight w2 = to_log(final);
    ...
    for (fst::ArcIterator<fst::Fst<Arc>> aiter(*fst, s); !aiter.Done();
      ...
      miter.Seek(bo_pos);
      Arc bo_arc = miter.Value();
      fst::Log64Weight w1 = to_log(bo_arc.weight);
      fst::Log64Weight w2 = to_log(arc.weight);
