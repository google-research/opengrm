...
#ifndef OPENGRM_SFST_RANDGEN_H_
#define OPENGRM_SFST_RANDGEN_H_
...
class SFstArcSelector {
 public:
  ...
  explicit SFstArcSelector(Label phi_label = fst::kNoLabel)
      : phi_label_(phi_label) {}

  // Samples one transition.
  size_t operator()(absl::BitGenRef bit_gen, const fst::Fst<A>& fst, StateId s,
    ...
    double r = absl::Uniform(bit_gen, 0.0, 1.0);
    ...
    if (failure_pos == -1 || r <= pre_failure_prob) {
      ...
      return accumulator->LowerBound(typename A::Weight(-flogd(r)), &aiter);
    }
...
class ArcSampler<A, sfst::SFstArcSelector<A>> {
 public:
  using Accumulator = CacheLogAccumulator<A>;
  using Label = typename A::Label;
  using StateId = typename A::StateId;
  using Weight = typename A::Weight;

  ArcSampler(const Fst<A>& fst, const sfst::SFstArcSelector<A>& arc_selector,
  ...
  ArcSampler(const ArcSampler<A, sfst::SFstArcSelector<A>>& sampler,
  ...
  bool Sample(absl::BitGenRef bit_gen, const RandState<A>& rstate);
 ...
 private:
  ...
  double PreFailureProb(StateId s, ssize_t failure_pos) {
    ...
    accumulator_->SetState(s);
    Weight cumul_weight =
        accumulator_->Sum(Weight::Zero(), &aiter, 0, failure_pos);
  ...
  void MultinomialSample(absl::BitGenRef bit_gen, const RandState<A>& rstate,
                         Weight fail_weight);

  const Fst<A>& fst_;
  const sfst::SFstArcSelector<A>& arc_selector_;
  int max_length_;
  Label phi_label_;
...
bool ArcSampler<A, sfst::SFstArcSelector<A>>::Sample(
    absl::BitGenRef bit_gen, const RandState<A>& rstate) {
  sample_map_.clear();
  ...
  if (fst_.NumArcs(rstate.state_id) + 1 < rstate.nsamples) {
    Weight numer_weight = to_weight_(Log64Weight(-log(numer_prob)));
    MultinomialSample(bit_gen, rstate, numer_weight);
    Reset();
    return true;
  }
  ...
  for (size_t i = 0; i < rstate.nsamples; ++i) {
    ...
    do {
      ...
      pos = arc_selector_(bit_gen, fst_, rstate.state_id, pre_failure_prob,
...
bool ArcSampler<A, sfst::SFstArcSelector<A>>::ForbiddenPosition(
  ...
  if (forbidden_positions_.count(pos) > 0) return true;
  ...
  if (forbidden_label) forbidden_positions_.insert(pos);
...
bool ArcSampler<A, sfst::SFstArcSelector<A>>::ForbiddenLabel(
  ...
  if (phi_label_ == kNoLabel || l == phi_label_ || l == 0) return false;
  if (fst_.NumArcs(rstate.state_id) > rstate.nsamples) {
    for (const RandState<A>* rs = &rstate; rs->parent != nullptr;
      ...
      // Not failure transition.
      if (aiter.Value().ilabel != phi_label_) return false;
      if (l == kNoLabel) {  // Super-final label.
        if (fst_.Final(parent_id) != Weight::Zero()) return true;
      } else {
    ...
    if (forbidden_labels_.empty()) {
      for (const RandState<A>* rs = &rstate; rs->parent != nullptr;
        ...
        aiter.Seek(rs->select);
        // Not failure transition.
        if (aiter.Value().ilabel != phi_label_) break;
        for (aiter.Reset(); !aiter.Done(); aiter.Next()) {
          Label l = aiter.Value().ilabel;
          if (l != phi_label_) forbidden_labels_.insert(l);
        }
...
void ArcSampler<A, sfst::SFstArcSelector<A>>::MultinomialSample(
    absl::BitGenRef bit_gen, const RandState<A>& rstate, Weight fail_weight) {
  pr_.clear();
  pos_.clear();
  n_.clear();
  size_t pos = 0;
  ...
  n_.resize(pr_.size());
  OneMultinomialSample(pr_, rstate.nsamples, &n_, bit_gen);
