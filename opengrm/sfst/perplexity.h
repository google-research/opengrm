...
#ifndef OPENGRM_SFST_PERPLEXITY_H_
#define OPENGRM_SFST_PERPLEXITY_H_
...
class WeightTransform {
 public:
  ...
  EWeight operator()(const Weight& w) const {
    if (w == Weight::Zero()) return EWeight::Zero();
...
struct WeightConvert<Entropy64Weight, Log64Weight> {
  ...
  LWeight operator()(const EWeight& w) const { return w.Value1().Value2(); }
};
...
struct WeightConvert<Log64Weight, Entropy64Weight> {
  ...
  EWeight operator()(const LWeight& w) const {
    SLWeight slw(SLWeight::W1(1.0), SLWeight::W2(w.Value()));
...
inline Entropy64Weight Minus(Entropy64Weight w1, Entropy64Weight w2) {
  ...
  return Plus(
      w1, Entropy64Weight(Minus(slz, w2.Value1()), Minus(rlz, w2.Value2())));
}
...
class Entropy64WeightApproxEqual {
 public:
  ...
  explicit Entropy64WeightApproxEqual(float delta)
      : slw_equal_(delta), rw_equal_(delta) {}
...
class Perplexity {
 public:
  ...
  explicit Perplexity(const fst::Fst<Arc>& fst, Label phi_label = fst::kNoLabel,
  ...
  double SkipCount() const { return sent_count_ - GetSourceCount(); }
 ...
 private:
  ...
  void FindVocabSet(const fst::Fst<Arc>& sfst) {
    ...
    for (fst::StateIterator<fst::Fst<Arc>> siter(sfst); !siter.Done();
         siter.Next()) {
      StateId s = siter.Value();
      for (fst::ArcIterator<fst::Fst<Arc>> aiter(sfst, s); !aiter.Done();
           aiter.Next()) {
  ...
  void PrepareSource(const fst::Fst<Arc>& ifst, fst::MutableFst<EArc>* ofst);

  // Self or cross entropy being computed?
  bool IsSelfEntropy() const { return qfst_.Start() == fst::kNoStateId; }

  // Returns accumulated cross/self entropy since construction
  // or last reset.
  double GetTotalEntropy() const { return entropy_.Sum().Value2().Value(); }
  ...
  absl::flat_hash_set<Label> vocab_;
...
void Perplexity<Arc>::SetTarget(const fst::Fst<Arc>& ifst) {
  ...
  WCM wc_mapper(to_e);
  fst::ArcMap(ifst, &qfst_, wc_mapper);

  if (unknown_label_ != fst::kNoLabel) FindVocabSet(ifst);
...
void Perplexity<Arc>::PrepareSource(const fst::Fst<Arc>& ifst,
  ...
  const auto etype = (IsSelfEntropy() ? WT::ENTROPY : WT::CROSS_ENTROPY_SOURCE);

  WT to_e(etype);
  WCM wc_mapper(to_e);
  fst::ArcMap(ifst, ofst, wc_mapper);
  fst_oov_count_ = 0;

  if (unknown_label_ == fst::kNoLabel || IsSelfEntropy()) return;

  for (StateId s = 0; s < ofst->NumStates(); ++s) {
    for (fst::MutableArcIterator<fst::MutableFst<EArc>> aiter(ofst, s);
...
bool Perplexity<Arc>::Apply(const fst::Fst<Arc>& fst) {
  ...
  fst::VectorFst<EArc> pfst, plogq_fst;
  if (IsSelfEntropy()) {
  ...
  EWeight entropy = ShortestDistance<EArc, EArc, WEq>(
      plogq_fst, &distance, phi_label_, false, entropy_delta_);

  // To address a corner case where higher-indexed states are effectively
  // unreachable (very high cost). This ensures that all states will have
  // distance populated.
  distance.resize(plogq_fst.NumStates(), EWeight::Zero());
  ...
  if (entropy.Member() && entropy != EWeight::Zero()) {
    ...
    for (auto w : distance) state_count_.Add(w.Value1());
  }
  return true;
}
...
#endif  // OPENGRM_SFST_PERPLEXITY_H_
