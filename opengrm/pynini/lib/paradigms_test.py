...
class LatinFirstDeclensionNounTest(absltest.TestCase):
  ...
  def testGenerator(self):
    ...
    self.assertSameElements(
        [
            "aqu+a[case=nom][num=sg]",
            "aqu+ae[case=gen][num=sg]",
            "aqu+ae[case=dat][num=sg]",
            "aqu+am[case=acc][num=sg]",
            "aqu+ā[case=abl][num=sg]",
            "aqu+ae[case=nom][num=pl]",
            "aqu+ārum[case=gen][num=pl]",
            "aqu+īs[case=dat][num=pl]",
            "aqu+ās[case=acc][num=pl]",
            "aqu+īs[case=abl][num=pl]",
        ],
        forms,
    )
    ...
    self.assertSameElements(
        [(
            ...
            features.FeatureVector(
                self.paradigm.category, "case=gen", "num=pl"
            ),
        )],
    ...
    self.assertSameElements(
        [
            (
                "puell+īs",
                features.FeatureVector(
                    self.paradigm.category, "case=dat", "num=pl"
                ),
            ),
            (
                "puell+īs",
                features.FeatureVector(
                    self.paradigm.category, "case=abl", "num=pl"
                ),
            ),
        ],
  ...
  def testGenerator(self):
    ...
    self.assertSameElements(
        [
            ...
            "noct+um[case=gen][num=pl]",  # TODO: Actually "noctium".
    ...
    self.assertSameElements(
        [
            "rēx+[case=nom][num=sg]",
            "rēg+is[case=gen][num=sg]",
            "rēg+ī[case=dat][num=sg]",
            "rēg+em[case=acc][num=sg]",
            "rēg+e[case=abl][num=sg]",
            "rēg+ēs[case=nom][num=pl]",
            "rēg+um[case=gen][num=pl]",
            "rēg+ibus[case=dat][num=pl]",
            "rēg+ēs[case=acc][num=pl]",
            "rēg+ibus[case=abl][num=pl]",
        ],
        forms,
    )
    ...
    self.assertSameElements(
        [
            "ōs+[case=nom][num=sg]",
            "ōr+is[case=gen][num=sg]",
            "ōr+ī[case=dat][num=sg]",
            "ōr+em[case=acc][num=sg]",
            "ōr+e[case=abl][num=sg]",
            "ōr+ēs[case=nom][num=pl]",
            "ōr+um[case=gen][num=pl]",
            "ōr+ibus[case=dat][num=pl]",
            "ōr+ēs[case=acc][num=pl]",
            "ōr+ibus[case=abl][num=pl]",
        ],
        forms,
    )
  ...
  def testAnalyzer(self):
    self.assertSameElements(
        [(
            "ōs+",
            features.FeatureVector(
                self.paradigm.category, "case=nom", "num=sg"
            ),
        )],
    ...
    self.assertSameElements(
        [(
            "rēg+e",
            features.FeatureVector(
                self.paradigm.category, "case=abl", "num=sg"
            ),
        )],
  ...
  def testLemmatizer(self):
    self.assertSameElements(
        [(
            "pax",
            features.FeatureVector(
                self.paradigm.category, "case=acc", "num=sg"
            ),
        )],
...
class LatinThirdDeclensionNounStemIdsTest(absltest.TestCase):
  ...
  def setUpClass(cls):
    ...
    rules = [
        ...
        # s -> r / V __ V.
        pynini.cdrewrite(
            pynini.cross("s", "r"), "", stem_ids + "+" + v, cls.noun.sigma_star
        ),
        # s+s -> s.
        pynini.cdrewrite(
            pynini.cross("s", ""), "s" + stem_ids + "+", "", cls.noun.sigma_star
        ),
    ]
    cls.paradigm = paradigms.Paradigm(
        category=cls.noun,
  ...
  def testGenerator(self):
    ...
    self.assertSameElements(
        [
            "ōs__1001__+[case=nom][num=sg]",
            "ōr__1001__+is[case=gen][num=sg]",
  ...
  def testFindForm(self):
    filt = (
        self.noun.sigma_star
        + "__1001__"
        + self.noun.sigma_star
        + "[case=acc][num=sg]"
    )
    forms = (
        self.paradigm.stems_to_forms
        @ filt
        @ self.delete_stem_ids
        @ self.paradigm.feature_label_rewriter
    )
    self.assertSameElements(
        ["ōr+em[case=acc][num=sg]"], forms.optimize().paths().ostrings()
    )
    filt = (
        self.noun.sigma_star
        + "__1002__"
    ...
    self.assertSameElements(
        ["pac+um[case=gen][num=pl]"], forms.optimize().paths().ostrings()
    )
...
class TagalogUmInfixationTest(absltest.TestCase):
  ...
  def testGenerate(self):
    ...
    self.assertSameElements(
        ["bilang[focus=none]", "b+um+ilang[focus=actor]"],
        form.paths().ostrings(),
    )
    form = (
        "ibig"
    ...
    self.assertSameElements(
        ["ibig[focus=none]", "um+ibig[focus=actor]"], form.paths().ostrings()
    )
...
class YowlumneVerbalAspectTest(absltest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cvcc = (
        c + v + pynutil.delete(v).ques + c + pynutil.delete(v).star + c.ques
    ).optimize()
    ...
    for v in vowels:
      cvcvvc.union(
          c
          + v
          + pynutil.delete(v).ques
          + c
          + pynutil.delete(v).star
          + pynutil.insert(v + v)
          + c.ques
      )
    cvcvvc.optimize()
  ...
  def testStems(self):
    stems_and_forms = [
        (
            "caw",
            [
                "caw+al[aspect=dubitative]",
                "caw+inay[aspect=gerundial]",
                "cawaa+ʔaa[aspect=durative]",
                "caw[aspect=root]",
            ],
        ),
        (
            "cuum",
            [
                "cuum+al[aspect=dubitative]",
                "cum+inay[aspect=gerundial]",
                "cumuu+ʔaa[aspect=durative]",
                "cuum[aspect=root]",
            ],
        ),
        (
            "diiyl",
            [
                "diiyl+al[aspect=dubitative]",
                "diyl+inay[aspect=gerundial]",
                "diyiil+ʔaa[aspect=durative]",
                "diiyl[aspect=root]",
            ],
        ),
        (
            "hiwiit",
            [
                "hiwiit+al[aspect=dubitative]",
                "hiwt+inay[aspect=gerundial]",
                "hiwiit+ʔaa[aspect=durative]",
                "hiwiit[aspect=root]",
            ],
        ),
        (
            "hoyoo",
            [
                "hoyoo+al[aspect=dubitative]",
                "hoy+inay[aspect=gerundial]",
                "hoyoo+ʔaa[aspect=durative]",
                "hoyoo[aspect=root]",
            ],
        ),
        (
            "ʔilk",
            [
                "ʔilk+al[aspect=dubitative]",
                "ʔilk+inay[aspect=gerundial]",
                "ʔiliik+ʔaa[aspect=durative]",
                "ʔilk[aspect=root]",
            ],
        ),
    ]
  ...
  def testGenerator(self):
    ...
    self.assertSameElements(
        [
            ...
            "stol+á[case=gen][num=sg]",
            "stol+ú[case=dat][num=sg]",
            "stól[case=acc][num=sg]",
            "stol+óm[case=ins][num=sg]",
            "stol+é[case=prp][num=sg]",
            "stol+óv[case=gen][num=pl]",
            "stol+ý[case=acc][num=pl]",
            "stol+ý[case=nom][num=pl]",
            "stol+ám[case=dat][num=pl]",
            "stol+ámi[case=ins][num=pl]",
            "stol+áx[case=prp][num=pl]",
        ],
        forms,
    )
  ...
  def testAnalyzer(self):
    self.assertSameElements(
        [(
            "grádus+ov",
            features.FeatureVector(
                self.paradigm_a.category, "case=gen", "num=pl"
            ),
        )],
    ...
    self.assertSameElements(
        [(
            "stol+óv",
            features.FeatureVector(
                self.paradigm_b.category, "case=gen", "num=pl"
            ),
        )],
