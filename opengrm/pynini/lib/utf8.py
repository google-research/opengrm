...
def _byte_range(min_val, max_val=None) -> pynini.Fst:
  if max_val is None:
    max_val = min_val
  return pynini.union(
      *(f"[{i}]" for i in range(min_val, max_val + 1))
  ).optimize()
...
# codepoint.
LEADING_BYTE = pynini.union(
    _leading_byte_2_byte, _leading_byte_3_byte, _leading_byte_4_byte
).optimize()
...
VALID_BYTE = pynini.union(
    SINGLE_BYTE, LEADING_BYTE, CONTINUATION_BYTE
).optimize()
...
VALID_UTF8_CHAR_REGIONAL_INDICATOR_SYMBOL = (
    "[240][159][135]" + pynini.union(*(f"[{i}]" for i in range(166, 192)))
).optimize()
