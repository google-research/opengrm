...
def build_stem_ids(min_id: int, max_id: int) -> pynini.Fst:
  ...
  """
  return pynini.union(
      *["__{}__".format(i) for i in range(min_id, max_id)]
  ).optimize()


def make_byte_star_except_boundary(
    boundary: pynini.FstLike = "+",
) -> pynini.Fst:
...
class Paradigm:
  ...
  def __init__(
    ...
    Args:
      ...
      stems: a sequence of strings and/or acceptors representing stems belonging
        to this paradigm.
    ...
    for _, feature_vector in self._slots:
      if feature_vector.category != self._category:
        raise Error(
            f"Paradigm category {self._category} != "
  ...
  def _inherit(self) -> None:
    ...
    if self._boundary != self._parent_paradigm.boundary:
      raise Error(
          f"Paradigm boundary {self._boundary} != "
    ...
    self._slots.extend(
        parent_slot
        for parent_slot in self._parent_paradigm.slots
