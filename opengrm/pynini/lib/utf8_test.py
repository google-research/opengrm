...
class Utf8Test(absltest.TestCase):
  ...
  def all_suffixes(self, fsa: pynini.Fst) -> pynini.Fst:
    ...
    for s in fsa.states():
      fsa.add_arc(
          start_state, pynini.Arc(0, 0, pynini.Weight.one(fsa.weight_type()), s)
      )
    return fsa.optimize()
  ...
  def assertStrInAcceptorLanguage(
      self, string: pynini.FstLike, fst: pynini.Fst
  ) -> None:
  ...
  def assertStrNotInAcceptorLanguage(
      self, string: pynini.FstLike, fst: pynini.Fst
  ) -> None:
    self.assertIsFsa(fst)
  ...
  def testVerifyLeadingBytesDefinition(self):
    ...
    leading_bytes = (
        self.all_single_byte_prefixes(utf8.VALID_UTF8_CHAR) - utf8.SINGLE_BYTE
    )
  ...
  def testVerifyContinuationBytesDefinition(self):
    ...
    # characters barring the legal single byte characters (ASCII).
    continue_bytes = (
        self.all_single_byte_suffixes(utf8.VALID_UTF8_CHAR) - utf8.SINGLE_BYTE
    )
  ...
  def testVerifyUtf8CharRegionalIndicatorSymbolDefinition(self):
    regional_indicator = pynini.string_map(
        ...
        # through 0x1F1FF.
        pynini.escape(chr(codepoint))
        for codepoint in range(0x1F1E6, 0x1F1FF + 1)
    ).optimize()
