...
class Feature:
  ...
  def __init__(
      self, name: str, *values: str, default: Optional[str] = None
  ) -> None:
    ...
    self._acceptor = pynini.union(
        *(f"[{self._name}={v}]" for v in self._values)
    )
...
class Category:
  ...
  def __init__(self, *features: Feature) -> None:
    ...
    self._sigma_star = (
        pynini.union(byte.BYTE, self._feature_labels).closure().optimize()
    )
