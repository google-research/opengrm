...
class DirectionalityTest(absltest.TestCase):
  ...
  def assertOneTopRewrite(
      self, istring: str, ostring: str, rule: pynini.Fst
  ) -> None:
  ...
  def testLeftToRightApplication(self):
    rule = pynini.cdrewrite(
        self.tau, "b", "b", self.sigstar, direction="ltr"
    ).optimize()
  ...
  def testRightToLeftApplication(self):
    rule = pynini.cdrewrite(
        self.tau, "b", "b", self.sigstar, direction="rtl"
    ).optimize()
    self.assertUnambiguousExamples(rule)
  ...
  def testSimultaneousApplication(self):
    rule = pynini.cdrewrite(
        self.tau, "b", "b", self.sigstar, direction="sim"
    ).optimize()
    self.assertUnambiguousExamples(rule)
...
class TDTest(absltest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.consonant = pynini.union(
        cls.td, "p", "b", "f", "v", "s", "z", "k", "g", "m", "n", "l", "r"
    ).optimize()

  def testMandatoryRewrite(self):
    rule = pynini.cdrewrite(
        pynutil.delete(self.td), self.consonant, "[EOS]", self.sigstar
    ).optimize()
...
class RankedTest(absltest.TestCase):
  ...
  def testRankedRewrite(self):
    deletion_rule = pynini.cdrewrite(
        pynutil.delete(self.consonant, weight=1),
        "",
        self.consonant,
        self.sigstar,
    )
    epenthesis_rule = pynini.cdrewrite(
        pynutil.insert("i", weight=2),
        self.consonant,
        self.consonant,
        self.sigstar,
    )
