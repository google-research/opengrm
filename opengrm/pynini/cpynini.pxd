...
from openfst.extensions.python.cpywrapfst cimport QueueType
from openfst.extensions.python.cpywrapfst cimport SymbolTable
from openfst.extensions.python.cpywrapfst cimport TokenType
...
cdef extern from "openfst/extensions/mpdt/compose.h" namespace "fst" nogil:

  cdef cppclass MPdtComposeOptions:

    MPdtComposeOptions(bool, PdtComposeFilter)

cdef extern from "openfst/extensions/mpdt/expand.h" namespace "fst" nogil:
...
cdef extern from "openfst/extensions/mpdt/mpdtscript.h" \
...
cdef extern from "openfst/extensions/mpdt/read_write_utils.h" \
    namespace "fst" nogil:
  ...
  # TODO: The last argument is actually const but externally Cython 0.28
...
cdef extern from "openfst/extensions/pdt/compose.h" namespace "fst" nogil:
...
cdef extern from "openfst/extensions/pdt/replace.h" namespace "fst" nogil:
...
cdef extern from "openfst/extensions/pdt/getters.h" \
    namespace "fst::script" nogil:

  bool GetPdtComposeFilter(const string &, PdtComposeFilter *)


cdef extern from "openfst/extensions/pdt/pdtscript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "openfst/lib/util.h" namespace "fst" nogil:
...
cdef extern from "opengrm/operators/cdrewrite.h" \
    namespace "fst" nogil:
  ...
  ctypedef enum CDRewriteMode:
    ...
    OPTIONAL "fst::CDRewriteMode::OPTIONAL"


cdef extern from "opengrm/operators/cdrewritescript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/operators/concatrangescript.h" \
    namespace "fst::script" nogil:

  void ConcatRange(MutableFstClass *, int32_t, int32_t)


cdef extern from "opengrm/operators/getters.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/operators/crossscript.h" \
    namespace "fst::script" nogil:

  void Cross(const FstClass &, const FstClass &, MutableFstClass *)

cdef extern from "opengrm/operators/lenientlycomposescript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/operators/optimizescript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/paths/pathsscript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/string/defaults.h" namespace "fst" nogil:
...
cdef extern from "opengrm/string/stringutil.h" \
    namespace "fst" nogil:

  string Escape(const string &)


cdef extern from "opengrm/string/stringcompile.h" \
    namespace "fst" nogil:
...
cdef extern from "opengrm/string/stringcompilescript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/string/stringmapscript.h" \
    namespace "fst::script" nogil:
...
cdef extern from "opengrm/string/stringprintscript.h" \
    namespace "fst::script" nogil:
