...
class Deabbreviator:
  ...
  _r_deletion = pynini.cdrewrite(
      pynutil.delete(_r), _v, _c - _r, _sigma_star, mode="opt"
  ).optimize()
  _v_deletion = pynini.cdrewrite(
      pynutil.delete(_vowel_span), _c, _c | "[EOS]", _sigma_star, mode="opt"
  ).optimize()
...
class Regexps:
  ...
  # TODO: The following regexps are based on an internal, now defunct, #
