...
_OUTPUTS = flags.DEFINE_string(
    'outputs',
    None,
    'The output FAR files in the form designator1=file1,designator2=file2,...',
)
FLAGS = flags.FLAGS
  ...
  for target_file_pair in target_file_pairs.split(','):
    ...
    if len(target_file_list) != 2:
      raise app.UsageError(
          '--outputs must be of form designator1=file1,designator2=file2,...'
      )
    result[target_file_list[0]] = target_file_list[1]
    logging.info(
        'Setting up target designator [%s] with file [%s].',
        target_file_list[0],
        target_file_list[1],
    )
  return result
