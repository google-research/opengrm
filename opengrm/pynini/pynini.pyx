...
from openfst.extensions.python.cpywrapfst cimport ArcSort
from openfst.extensions.python.cpywrapfst cimport ArcSortType
from openfst.extensions.python.cpywrapfst cimport Closure
from openfst.extensions.python.cpywrapfst cimport CLOSURE_PLUS
...
from openfst.extensions.python.pywrapfst cimport Fst as _Fst
from openfst.extensions.python.pywrapfst cimport MutableFst as \
    _MutableFst
from openfst.extensions.python.pywrapfst cimport SymbolTable_ptr
from openfst.extensions.python.pywrapfst cimport const_SymbolTable_ptr
from openfst.extensions.python.pywrapfst cimport VectorFst as _VectorFst
from openfst.extensions.python.pywrapfst cimport Weight as _Weight
from openfst.extensions.python.pywrapfst cimport SymbolTable as \
    _SymbolTable
from openfst.extensions.python.pywrapfst cimport SymbolTableView as \
    _SymbolTableView
from openfst.extensions.python.pywrapfst cimport _get_WeightClass_or_one
from openfst.extensions.python.pywrapfst cimport \
    _get_WeightClass_or_zero
from openfst.extensions.python.pywrapfst cimport _get_compose_filter
from openfst.extensions.python.pywrapfst cimport _get_queue_type
from openfst.extensions.python.pywrapfst cimport _get_replace_label_type
from openfst.extensions.python.pywrapfst cimport equal
from openfst.extensions.python.pywrapfst cimport replace as _replace
from openfst.extensions.python.pywrapfst cimport tostring
from openfst.extensions.python.pywrapfst cimport path_tostring

# C++ code for Pynini not from fst_util.

from opengrm.pynini.cpynini cimport CDRewriteCompile
from opengrm.pynini.cpynini cimport CDRewriteDirection as \
    _CDRewriteDirection
from opengrm.pynini.cpynini cimport CDRewriteMode as _CDRewriteMode
from opengrm.pynini.cpynini cimport Compose
from opengrm.pynini.cpynini cimport ConcatRange
from opengrm.pynini.cpynini cimport Cross
from opengrm.pynini.cpynini cimport Escape
from opengrm.pynini.cpynini cimport Expand
from opengrm.pynini.cpynini cimport GeneratedSymbols
from opengrm.pynini.cpynini cimport GetCDRewriteDirection
from opengrm.pynini.cpynini cimport GetDefaultSymbols
from opengrm.pynini.cpynini cimport GetDefaultTokenType
from opengrm.pynini.cpynini cimport GetCDRewriteMode
from opengrm.pynini.cpynini cimport GetPdtComposeFilter
from opengrm.pynini.cpynini cimport GetPdtParserType
from opengrm.pynini.cpynini cimport LenientlyCompose
from opengrm.pynini.cpynini cimport MPdtComposeOptions
from opengrm.pynini.cpynini cimport MPdtExpandOptions
from opengrm.pynini.cpynini cimport Optimize
from opengrm.pynini.cpynini cimport OptimizeDifferenceRhs
from opengrm.pynini.cpynini cimport PdtComposeFilter
from opengrm.pynini.cpynini cimport PdtComposeOptions
from opengrm.pynini.cpynini cimport PdtExpandOptions
from opengrm.pynini.cpynini cimport PdtParserType
from opengrm.pynini.cpynini cimport PdtShortestPathOptions
from opengrm.pynini.cpynini cimport Replace
from opengrm.pynini.cpynini cimport Reverse
from opengrm.pynini.cpynini cimport ReadLabelPairs
from opengrm.pynini.cpynini cimport ReadLabelTriples
from opengrm.pynini.cpynini cimport ShortestPath
from opengrm.pynini.cpynini cimport StringCompile
from opengrm.pynini.cpynini cimport PopDefaults
from opengrm.pynini.cpynini cimport PushDefaults
from opengrm.pynini.cpynini cimport StringFileCompile
from opengrm.pynini.cpynini cimport StringMapCompile
from opengrm.pynini.cpynini cimport StringPathIteratorClass
from opengrm.pynini.cpynini cimport StringPrint
from opengrm.pynini.cpynini cimport TokenType as _TokenType
from opengrm.pynini.cpynini cimport WriteLabelPairs
from opengrm.pynini.cpynini cimport WriteLabelTriples
from opengrm.pynini.cpynini cimport kBosIndex
from opengrm.pynini.cpynini cimport kEosIndex
...
from openfst.extensions.python.pywrapfst import FstArgError
from openfst.extensions.python.pywrapfst import FstIOError
from openfst.extensions.python.pywrapfst import FstOpError

from openfst.extensions.python import pywrapfst
...
cdef _TokenType _get_token_type(const string &token_type) except *:
  ...
  cdef bool _result
  with nogil:
    _result = GetTokenType(token_type, addr(_token_type))
  if not _result:
...
cdef _CDRewriteDirection _get_cdrewrite_direction(
  ...
  cdef bool _result
  with nogil:
    _result = GetCDRewriteDirection(direction, addr(_direction))
  if not _result:
    raise FstArgError(
...
cdef _CDRewriteMode _get_cdrewrite_mode(const string &mode) except *:
  ...
  cdef bool _result
  with nogil:
    _result = GetCDRewriteMode(mode, addr(_mode))
  if not _result:
    raise FstArgError(
...
cdef PdtComposeFilter _get_pdt_compose_filter(
  ...
  cdef bool _result
  with nogil:
    _result = GetPdtComposeFilter(compose_filter, addr(_compose_filter))
  if not _result:
...
cdef PdtParserType _get_pdt_parser_type(const string &pdt_parser_type) except *:
  ...
  cdef bool _result
  with nogil:
    _result = GetPdtParserType(pdt_parser_type, addr(_pdt_parser_type))
  if not _result:
...
class default_token_type(contextlib.ContextDecorator):
  ...
  def __enter__(self):
    ...
    if isinstance(self._token_type, pywrapfst.SymbolTableView):
...
cdef class Fst(_VectorFst):
  ...
  def __init__(self, arc_type="standard"):
    cdef unique_ptr[VectorFstClass] _fst
    _fst.reset(new VectorFstClass(tostring(arc_type)))
    if _fst.get().Properties(kError, True) == kError:
       raise FstArgError(f"Unknown arc type: {arc_type}")
    self._from_MutableFstClass(_fst.release())
  ...
  def from_pywrapfst(cls, _Fst fst):
    ...
    This class method converts an FST from the pywrapfst module (pywrapfst.Fst
  ...
  cpdef string string(self, token_type=None) except *:
    ...
    Args:
      token_type: An optional string indicating how the string is to be
          ...
          raw bytes), "utf8" (interprets arc labels as Unicode code points),
    ...
    elif isinstance(token_type, pywrapfst.SymbolTableView):
      _token_type = _TokenType.SYMBOL
    ...
    cdef bool _result
    with nogil:
      _result = StringPrint(deref(self._fst),
                            addr(_string),
                            _token_type,
                            _symbols)
    if not _result:
  ...
  cpdef Fst copy(self):
    ...
    """
    # TODO: Switch to Python 3-style zero-argument `super()` once
    # this is supported by Cython in `cpdef` methods.
    # See: https://github.com/cython/cython/issues/3726.
  ...
  def plus(self):
    ...
    Returns:
      An FST copy.
    """
    cdef Fst _fst = self.copy()
    with nogil:
      Closure(_fst._mfst.get(), CLOSURE_PLUS)
    _fst._check_mutating_imethod()
    return _fst
  ...
  def ques(self):
    ...
    cdef Fst _fst = self.copy()
    with nogil:
      ConcatRange(_fst._mfst.get(), 0, 1)
    _fst._check_mutating_imethod()
    return _fst
  ...
  def star(self):
    ...
    cdef Fst _fst = self.copy()
    with nogil:
      Closure(_fst._mfst.get(), CLOSURE_STAR)
    _fst._check_mutating_imethod()
    return _fst
  ...
  # TODO: Cython <3.0 only has support for two-argument __ipow__; see:
  #
...
cdef Fst _init_Fst_from_MutableFst(_MutableFst fst):
  cdef Fst _fst = Fst.__new__(Fst)
  _fst._fst = fst._fst
  _fst._mfst = fst._mfst
  return _fst
...
cpdef Fst _from_pywrapfst(_Fst fst):
  cdef Fst _fst = Fst.__new__(Fst)
  _fst._from_MutableFstClass(new VectorFstClass(deref(fst._fst)))
  return _fst
...
cpdef string escape(data):
  ...
  Returns:
    An escaped string.
  """
  cdef string _string = tostring(data)
  with nogil:
...
cpdef Fst accep(astring, weight=None, arc_type="standard", token_type=None):
  ...
  Args:
    ...
    token_type: An optional string indicating how the input string is to be
        encoded as arc labels---one of: "byte" (encodes strings as raw bytes),
        "utf8" (encodes strings as UTF-8-encoded Unicode strings), or
  ...
  """
  cdef Fst _fst = Fst(arc_type)
  cdef WeightClass _weight = _get_WeightClass_or_one(_fst.weight_type(),
                                                     weight)
  cdef _TokenType _token_type
  cdef const_SymbolTable_ptr _symbols = NULL
  if token_type is None:
  ...
  cdef bool _result
  with nogil:
    _result = StringCompile(_astring,
                            _fst._mfst.get(),
                            _token_type,
                            _symbols,
                            _weight)
...
cpdef Fst cross(fst1, fst2):
  ...
  Args:
    fst1: The input string, or an acceptor FST representing the upper
        language.
    fst2: The output string, or an acceptor FST representing the lower
        language.
  ...
  cdef Fst _fst = Fst(_fst1.arc_type())
  with nogil:
    Cross(deref(_fst1._fst), deref(_fst2._fst), _fst._mfst.get())
  _fst._check_mutating_imethod()
  return _fst
...
cpdef Fst cdrewrite(tau, l, r, sigma_star, direction="ltr", mode="obl"):
  ...
  cdef Fst _fst = Fst(arc_type)
  cdef _CDRewriteDirection _direction = _get_cdrewrite_direction(
    tostring(direction))
...
cpdef Fst leniently_compose(mu, nu, sigma_star, compose_filter="auto",
  ...
  cdef Fst _fst = Fst(_mu.arc_type())
  with nogil:
...
cpdef Fst string_file(filename,
  ...
  Args:
    ...
    input_token_type: An optional string indicating how the input strings are
        to be encoded as arc labels---one of: "byte" (encodes strings as raw
        bytes), "utf8" (encodes strings as UTF-8-encoded Unicode strings),
        or a SymbolTable. If not set, or set to None, the value is set to the
        default token_type, which begins as "byte", but can be overridden for
        regions of code using the default_token_type context manager.
  ...
  elif isinstance(input_token_type, pywrapfst.SymbolTableView):
  ...
  elif isinstance(output_token_type, pywrapfst.SymbolTableView):
  ...
  cdef Fst _fst = Fst(arc_type=arc_type)
  cdef bool _result
  cdef string _filename = path_tostring(filename)
  with nogil:
    _result = StringFileCompile(_filename,
                                _fst._mfst.get(),
                                _input_token_type,
                                _output_token_type,
                                _isymbols,
...
cpdef Fst string_map(lines,
  ...
  Args:
    ...
    input_token_type: An optional string indicating how the input strings are to
        be encoded as arc labels---one of: "byte" (encodes strings as raw
        bytes), "utf8" (encodes strings as UTF-8-encoded Unicode strings),
        or a SymbolTable. If not set, or set to None, the value is set to the
        default token_type, which begins as "byte", but can be overridden for
        regions of code using the default_token_type context manager.
  ...
  cdef bool _result
  with nogil:
    _result = StringMapCompile(_lines,
                               _fst._mfst.get(),
                               _input_token_type,
                               _output_token_type,
                               _isymbols,
...
cpdef Fst replace(pairs,
  ...
  input an iterable of pairs, where each pair is formed by a non-terminal label
  and its corresponding FST; the first pair FST is the root FST. The resulting
  FST is obtained by taking the root FST and recursively replacing each arc
  having a nonterminal as output label by its corresponding FST. More
  precisely, an arc from state s to state d with (nonterminal) output label n
  in this FST is replaced by redirecting this "call" arc to the initial state
  of a copy F of the FST for n, and adding "return" arcs from each final state
  of F to d. Optional arguments control how the call and return arcs are
  labeled; by default, the only non-epsilon label is placed on the call arc.
...
cdef class PdtParentheses:
  ...
  cpdef PdtParentheses copy(self):
    ...
    """
    cdef PdtParentheses _parens = PdtParentheses.__new__(PdtParentheses)
    _parens._parens = self._parens
    return _parens
  ...
  def read(cls, filename):
    ...
    cdef bool _result
    with nogil:
      _result = ReadLabelPairs[int64_t](_filename,
                                        addr(_parens._parens))
    if not _result:
  ...
  cpdef void write(self, filename) except *:
    ...
    cdef bool _result
    with nogil:
      _result = WriteLabelPairs[int64_t](_filename, self._parens)
    if not _result:
...
def pdt_expand(fst,
  ...
  Raises:
    FstOpError: Operation failed.
  """
  cdef Fst _tfst = _compile_or_copy_Fst(fst)
  cdef Fst _fst = Fst(_tfst.arc_type())
  cdef WeightClass _weight = _get_WeightClass_or_zero(_fst.weight_type(),
                                                      weight)
  cdef unique_ptr[PdtExpandOptions] _opts
  _opts.reset(new PdtExpandOptions(connect, keep_parentheses, _weight))
  with nogil:
    Expand(deref(_tfst._fst), parens._parens, _fst._mfst.get(), deref(_opts))
  _fst._check_mutating_imethod()
  return _fst
...
cdef object _pdt_replace(pairs,
  ...
  cdef int64_t _label
  cdef _Fst _tfst
  for _label, _tfst in pairs:
    _pairs.push_back(LabelFstClassPair(_label, _tfst._fst.get()))
  cdef Fst _fst = Fst(_pairs[0].second.ArcType())
  cdef PdtParentheses _parens = PdtParentheses()
  cdef PdtParserType _pdt_parser_type = _get_pdt_parser_type(
      tostring(pdt_parser_type))
  cdef string _left_paren_prefix = tostring(left_paren_prefix)
  cdef string _right_paren_prefix = tostring(right_paren_prefix)
  with nogil:
    Replace(_pairs,
            _fst._mfst.get(),
            addr(_parens._parens),
...
def pdt_replace(pairs,
  ...
  This operation performs the dynamic replacement of arcs in one FST with
  another FST, allowing the definition of a PDT analogues to RTNs. It takes as
  input an iterable of pairs, where each pair is formed by a non-terminal label
  and its corresponding FST; the first pair FST is the root FST. The resulting
  FST is obtained by taking the root FST and recursively replacing each arc
  having a nonterminal as output label by its corresponding FST. The output
...
cpdef Fst pdt_reverse(fst, PdtParentheses parens):
  ...
  cdef Fst _fst = Fst(_tfst.arc_type())
  with nogil:
    Reverse(deref(_tfst._fst), parens._parens, _fst._mfst.get())
  _fst._check_mutating_imethod()
  return _fst
...
cdef class MPdtParentheses:
  ...
  cpdef MPdtParentheses copy(self):
    ...
    """
    cdef MPdtParentheses _parens = MPdtParentheses.__new__(MPdtParentheses)
    _parens._parens = self._parens
    _parens._assign = self._assign
    return _parens
  ...
  def read(cls, filename):
    ...
    cdef bool _result
    with nogil:
      _result = ReadLabelTriples[int64_t](_filename,
                                          addr(_parens._parens),
                                          addr(_parens._assign))
    if not _result:
      raise FstIOError(f"Read failed: {filename}")
    return _parens
  ...
  cpdef void write(self, filename) except *:
    ...
    cdef bool _result
    with nogil:
      _result = WriteLabelTriples[int64_t](_filename,
                                           self._parens,
                                           self._assign)
    if not _result:
      raise FstIOError(f"Write failed: {filename}")
...
def mpdt_reverse(fst, MPdtParentheses parens):
  ...
  cdef Fst _tfst = _compile_or_copy_Fst(fst)
  cdef Fst _fst = Fst(_tfst.arc_type())
  cdef MPdtParentheses _parens = parens.copy()
  with nogil:
    Reverse(deref(_tfst._fst),
            _parens._parens,
            addr(_parens._assign),
            _fst._mfst.get())
  _fst._check_mutating_imethod()
...
cdef class _StringPathIterator:
  ...
  def ilabels(self):
    ...
    """
    cdef vector[int64_t] _ilabels
    with nogil:
      _ilabels = self._paths.get().ILabels()
    return _ilabels
  ...
  def olabels(self):
    ...
    """
    cdef vector[int64_t] _olabels
    with nogil:
      _olabels = self._paths.get().OLabels()
    return _olabels
  ...
  cpdef string istring(self):
    ...
    """
    cdef string _istring
    with nogil:
      _istring = self._paths.get().IString()
    return _istring
  ...
  cpdef string ostring(self):
    ...
    """
    cdef string _ostring
    with nogil:
      _ostring = self._paths.get().OString()
    return _ostring
  ...
  cpdef _Weight weight(self):
    ...
    Returns:
      The path's Weight.
    """
    cdef _Weight _weight = _Weight.__new__(_Weight)
    with nogil:
      _weight._weight.reset(new WeightClass(self._paths.get().Weight()))
    return _weight
...
from openfst.extensions.python.pywrapfst import Arc
from openfst.extensions.python.pywrapfst import EncodeMapper
from openfst.extensions.python.pywrapfst import SymbolTable
from openfst.extensions.python.pywrapfst import SymbolTableView
from openfst.extensions.python.pywrapfst import Weight
from openfst.extensions.python.pywrapfst import _ArcIterator
from openfst.extensions.python.pywrapfst import _MutableArcIterator
from openfst.extensions.python.pywrapfst import _StateIterator


# Exceptions not yet imported.


from openfst.extensions.python.pywrapfst import FstBadWeightError
from openfst.extensions.python.pywrapfst import FstIndexError


# FST constants.


from openfst.extensions.python.pywrapfst import NO_LABEL
from openfst.extensions.python.pywrapfst import NO_STATE_ID
from openfst.extensions.python.pywrapfst import NO_SYMBOL


# FST properties.


from openfst.extensions.python.pywrapfst import ACCEPTOR
from openfst.extensions.python.pywrapfst import ACCESSIBLE
from openfst.extensions.python.pywrapfst import ACYCLIC
from openfst.extensions.python.pywrapfst import ADD_ARC_PROPERTIES
from openfst.extensions.python.pywrapfst import ADD_STATE_PROPERTIES
from openfst.extensions.python.pywrapfst import \
    ADD_SUPERFINAL_PROPERTIES
from openfst.extensions.python.pywrapfst import ARC_SORT_PROPERTIES
from openfst.extensions.python.pywrapfst import BINARY_PROPERTIES
from openfst.extensions.python.pywrapfst import COACCESSIBLE
from openfst.extensions.python.pywrapfst import COPY_PROPERTIES
from openfst.extensions.python.pywrapfst import CYCLIC
from openfst.extensions.python.pywrapfst import DELETE_ARC_PROPERTIES
from openfst.extensions.python.pywrapfst import DELETE_STATE_PROPERTIES
from openfst.extensions.python.pywrapfst import EPSILONS
from openfst.extensions.python.pywrapfst import ERROR
from openfst.extensions.python.pywrapfst import EXPANDED
from openfst.extensions.python.pywrapfst import EXTRINSIC_PROPERTIES
from openfst.extensions.python.pywrapfst import FST_PROPERTIES
from openfst.extensions.python.pywrapfst import FstProperties
from openfst.extensions.python.pywrapfst import INITIAL_ACYCLIC
from openfst.extensions.python.pywrapfst import INITIAL_CYCLIC
from openfst.extensions.python.pywrapfst import INTRINSIC_PROPERTIES
from openfst.extensions.python.pywrapfst import I_DETERMINISTIC
from openfst.extensions.python.pywrapfst import I_EPSILONS
from openfst.extensions.python.pywrapfst import \
    I_LABEL_INVARIANT_PROPERTIES
from openfst.extensions.python.pywrapfst import I_LABEL_SORTED
from openfst.extensions.python.pywrapfst import MUTABLE
from openfst.extensions.python.pywrapfst import NEG_TRINARY_PROPERTIES
from openfst.extensions.python.pywrapfst import NON_I_DETERMINISTIC
from openfst.extensions.python.pywrapfst import NON_O_DETERMINISTIC
from openfst.extensions.python.pywrapfst import NOT_ACCEPTOR
from openfst.extensions.python.pywrapfst import NOT_ACCESSIBLE
from openfst.extensions.python.pywrapfst import NOT_COACCESSIBLE
from openfst.extensions.python.pywrapfst import NOT_I_LABEL_SORTED
from openfst.extensions.python.pywrapfst import NOT_O_LABEL_SORTED
from openfst.extensions.python.pywrapfst import NOT_STRING
from openfst.extensions.python.pywrapfst import NOT_TOP_SORTED
from openfst.extensions.python.pywrapfst import NO_EPSILONS
from openfst.extensions.python.pywrapfst import NO_I_EPSILONS
from openfst.extensions.python.pywrapfst import NO_O_EPSILONS
from openfst.extensions.python.pywrapfst import NULL_PROPERTIES
from openfst.extensions.python.pywrapfst import O_DETERMINISTIC
from openfst.extensions.python.pywrapfst import O_EPSILONS
from openfst.extensions.python.pywrapfst import \
    O_LABEL_INVARIANT_PROPERTIES
from openfst.extensions.python.pywrapfst import O_LABEL_SORTED
from openfst.extensions.python.pywrapfst import POS_TRINARY_PROPERTIES
from openfst.extensions.python.pywrapfst import RM_SUPERFINAL_PROPERTIES
from openfst.extensions.python.pywrapfst import SET_ARC_PROPERTIES
from openfst.extensions.python.pywrapfst import SET_FINAL_PROPERTIES
from openfst.extensions.python.pywrapfst import SET_START_PROPERTIES
from openfst.extensions.python.pywrapfst import STATE_SORT_PROPERTIES
from openfst.extensions.python.pywrapfst import STRING
from openfst.extensions.python.pywrapfst import TOP_SORTED
from openfst.extensions.python.pywrapfst import TRINARY_PROPERTIES
from openfst.extensions.python.pywrapfst import UNWEIGHTED
from openfst.extensions.python.pywrapfst import UNWEIGHTED_CYCLES
from openfst.extensions.python.pywrapfst import WEIGHTED
from openfst.extensions.python.pywrapfst import WEIGHTED_CYCLES
from openfst.extensions.python.pywrapfst import \
...
from openfst.extensions.python.pywrapfst import ARC_FLAGS
from openfst.extensions.python.pywrapfst import ARC_I_LABEL_VALUE
from openfst.extensions.python.pywrapfst import ARC_NEXT_STATE_VALUE
from openfst.extensions.python.pywrapfst import ARC_NO_CACHE
from openfst.extensions.python.pywrapfst import ARC_O_LABEL_VALUE
from openfst.extensions.python.pywrapfst import ARC_VALUE_FLAGS
from openfst.extensions.python.pywrapfst import ARC_WEIGHT_VALUE


# Encode mapper properties.


from openfst.extensions.python.pywrapfst import ENCODE_FLAGS
from openfst.extensions.python.pywrapfst import ENCODE_LABELS
from openfst.extensions.python.pywrapfst import ENCODE_WEIGHTS
...
def _copy_patch(fnc):
  ...
  def patch(arg1, *args, **kwargs):
    cdef Fst _fst = _compile_or_copy_Fst(arg1)
    fnc(_fst, *args, **kwargs)
    return _fst
  return patch
...
from openfst.extensions.python.pywrapfst import compact_symbol_table
from openfst.extensions.python.pywrapfst import merge_symbol_table


# Weight operations.


from openfst.extensions.python.pywrapfst import divide
from openfst.extensions.python.pywrapfst import power
from openfst.extensions.python.pywrapfst import plus
from openfst.extensions.python.pywrapfst import times


# Custom types.

from openfst.extensions.python.pywrapfst import ArcMapType
from openfst.extensions.python.pywrapfst import ComposeFilter
from openfst.extensions.python.pywrapfst import DeterminizeType
from openfst.extensions.python.pywrapfst import EpsNormalizeType
from openfst.extensions.python.pywrapfst import FarType
from openfst.extensions.python.pywrapfst import ProjectType
from openfst.extensions.python.pywrapfst import QueueType
from openfst.extensions.python.pywrapfst import RandArcSelection
from openfst.extensions.python.pywrapfst import ReplaceLabelType
from openfst.extensions.python.pywrapfst import ReweightType
from openfst.extensions.python.pywrapfst import SortType
