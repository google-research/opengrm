...
#ifndef OPENGRM_REWRITE_REWRITE_MANAGER_H_
#define OPENGRM_REWRITE_REWRITE_MANAGER_H_
...
template <class Arc>
absl::Status RewriteManager<Arc>::LoadWithStatus(absl::string_view filename) {
  Reset();
  absl::StatusOr<std::unique_ptr<::fst::STTableFarReader<Arc>>> reader_statusor(
      ::fst::STTableFarReader<Arc>::OpenWithStatus(filename));
  if (!reader_statusor.ok()) return reader_statusor.status();
  std::unique_ptr<::fst::STTableFarReader<Arc>> reader =
      *std::move(reader_statusor);
...
#endif  // OPENGRM_REWRITE_REWRITE_MANAGER_H_
