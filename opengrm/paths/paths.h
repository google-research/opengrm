...
#ifndef OPENGRM_PATHS_PATHS_H_
#define OPENGRM_PATHS_PATHS_H_
...
void StringPathIterator<Arc>::IString(std::string* str) {
  if (!LabelsToString<Label>(ILabels(), str, input_token_type_, isymbols_)) {
    SetError();
  }
}
...
void StringPathIterator<Arc>::OString(std::string* str) {
  if (!LabelsToString<Label>(OLabels(), str, output_token_type_, osymbols_)) {
    SetError();
  }
}
